                                           ...
                                           const char *group)
{
    ...
    for (YAML::const_iterator itr = configyml[group].begin(); itr != configyml[group].end();
        ...
        if (paramKey == "config-file-path") {
            ...
            if (!get_absolute_file_path_yaml(app_cfg_file_path, str, config_file_path)) {
...
static void parse_streammux_width_height_yaml(gint *width, gint *height, gchar *cfg_file_path)
{
...
static void parse_sink_type_yaml(gint *type, gchar *cfg_file_path)
{
    YAML::Node configyml = YAML::LoadFile(cfg_file_path);

    for (YAML::const_iterator itr = configyml["sink"].begin(); itr != configyml["sink"].end();
         ++itr) {
        std::string paramKey = itr->first.as<std::string>();
...
std::vector<float> logits_to_probabilities(float *array, int arrayLen)
{
    ...
    if (!strncmp(name, "video", 5)) {
        ...
        if (bin_struct->vidconv) {
            ...
            if (gst_pad_link(decoder_src_pad, conv_sink_pad)) {
                g_printerr(
                    "Failed to link decoderbin src pad to"
                    " converter sink pad\n");
            }
                                  ...
                                  gpointer user_data)
{
    ...
    if (g_strstr_len(name, -1, "pngdec") == name) {
        ...
        bin_struct->capsraw = gst_element_factory_make("capsfilter", "raw_caps");
        ...
        gst_bin_add_many(GST_BIN(bin_struct->source_bin), bin_struct->vidconv, bin_struct->capsraw,
                         NULL);
    } else {
...
static bool create_source_bin(DsSourceBinStruct *ds_source_struct, gchar *uri)
{
    ...
    g_signal_connect(G_OBJECT(ds_source_struct->uri_decode_bin), "pad-added", G_CALLBACK(cb_newpad),
                     ds_source_struct);
...
int main(int argc, char *argv[])
{
    ...
    for (iterator = g_list, num_sources = 0; iterator; iterator = iterator->next, num_sources++) {
        ...
        srcpad = gst_element_get_static_pad(source_struct[num_sources].source_bin, pad_name_src);
        if (!srcpad) {
    ...
    if (sgie1_type == NVDS_GIE_PLUGIN_INFER_SERVER) {
        sgie1 =
            gst_element_factory_make("nvinferserver", "bodypose-classification-nvinference-engine");
    } else {
    ...
    gst_bin_add_many(GST_BIN(pipeline), nvvideoconvert_enlarge, capsFilter_enlarge, pgie, tracker,
                     sgie, preprocess1, sgie1, nvvidconv, nvosd, sink, nvdslogger,
                     nvvideoconvert_reduce, capsFilter_reduce, NULL);

    // Link elements
    if (!gst_element_link_many(streammux, nvvideoconvert_enlarge, capsFilter_enlarge, pgie, tracker,
                               sgie, preprocess1, sgie1, nvdslogger, nvvidconv, nvosd,
                               nvvideoconvert_reduce, capsFilter_reduce, sink, NULL)) {

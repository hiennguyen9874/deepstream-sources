...
SObjectContex *findUnusedObjectCtx(CustomCtx *ctx, guint64 object_id)
{
    std::unique_lock<std::mutex> lck(ctx->mtx);
    SObjectContex *pSObjectCtx = NULL;
    for (vector<SObjectContex *>::iterator itor = ctx->multi_objects.begin();
...
/* extend object group */
SObjectContex *CreateObjectCtx(CustomCtx *ctx)
{
    std::unique_lock<std::mutex> lck(ctx->mtx);
...
CustomCtx *initLib(CustomInitParams initparams)
{
    ...
    std::string sframeSeqLen =
        initparams.user_configs[NVDSPREPROCESS_USER_CONFIGS_FRAMES_SEQUENCE_LENGHTH];

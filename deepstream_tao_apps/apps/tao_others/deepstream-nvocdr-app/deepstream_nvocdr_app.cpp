...
{
    GMainLoop *loop = NULL;
    GstElement *streammux = NULL, *sink = NULL, *nvvidconv = NULL, *nvosd = NULL,
               *nvvidconv1 = NULL, *outenc = NULL, *capfilt = NULL, *nvtile = NULL,
    ...
    nvocdr_template = gst_element_factory_make("nvdsvideotemplate", "nvocdr_template");
    ...
    gst_bin_add_many(GST_BIN(pipeline), nvocdr_template, nvvidconv, nvosd, nvtile, sink, NULL);
    ...
    if (output_type == 1) {
        ...
        if (!isImage) {
            gst_bin_add_many(GST_BIN(pipeline), nvvidconv1, outenc, capfilt, encparse, mux, NULL);
            if (!gst_element_link_many(nvosd, nvvidconv1, capfilt, outenc, encparse, mux, sink,
                                       NULL)) {
                g_printerr("OSD and sink elements link failure.\n");
                return -1;
            }
        } else {
            gst_bin_add_many(GST_BIN(pipeline), nvvidconv1, outenc, capfilt, NULL);
            if (!gst_element_link_many(nvosd, nvvidconv1, capfilt, outenc, sink, NULL)) {
                g_printerr("OSD and sink elements link failure.\n");
                return -1;
            }
        }

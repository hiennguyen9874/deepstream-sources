 ...
 * APP_CONFIG_ANALYTICS_RESNET_PGIE_3SGIE_TYPE_COLOR_MAKE is applicable as sample demo code for
...
static GstClockTime generate_ts_rfc3339_from_ts(char *buf,
                                                int buf_size,
                                                GstClockTime ts,
                                                gchar *src_uri,
                                                gint stream_id)
    ...
    if (playback_utc ||
        ...
        if (testAppCtx->streams[stream_id].meta_number == 0) {
            ...
            memcpy(&tloc, (void *)(&testAppCtx->streams[stream_id].timespec_first_frame.tv_sec),
                   sizeof(time_t));
            ms = testAppCtx->streams[stream_id].timespec_first_frame.tv_nsec / 1000000;
...
static gpointer meta_copy_func(gpointer data, gpointer user_data)
{
    ...
    if (srcMeta->extMsgSize > 0) {
        if (srcMeta->objType == NVDS_OBJECT_TYPE_PERSON) {
            ...
            NvDsProductObject *obj = (NvDsProductObject *)g_malloc0(sizeof(NvDsProductObject));
            if (srcObj->brand)
                                    ...
                                    NvDsBatchMeta *batch_meta)
{
    ...
    if (tensor_meta) {
        ...
        if (model_used == APP_CONFIG_ANALYTICS_RETAIL) {
            ...
            meta->embedding.embedding_vector = (float *)g_malloc0(numElements * sizeof(float));
            cudaMemcpy(meta->embedding.embedding_vector,
                       (float *)(tensor_meta->out_buf_ptrs_dev[0]), numElements * sizeof(float),
            ...
            float *outputCoverageBuffer = (float *)tensor_meta->out_buf_ptrs_host[1];
                                                 ...
                                                 guint index)
{
    ...
    for (NvDsMetaList *l_frame = batch_meta->frame_meta_list; l_frame != NULL;
        ...
        scaleW =
            (float)frame_meta->source_frame_width / appCtx->config.streammux_config.pipeline_width;
        ...
        for (l = frame_meta->obj_meta_list; l != NULL; l = l->next) {
            ...
            generate_event_msg_meta(appCtx, msg_meta, obj_meta->class_id, TRUE, buffer_pts,
                                    appCtx->config.multi_source_config[stream_id].uri, stream_id,
                                    stream_id, obj_meta, scaleW, scaleH, frame_meta, tensor_meta,
                                    batch_meta);
            ...
            new_user_meta->base_meta.release_func = (NvDsMetaReleaseFunc)meta_free_func;
 ...
 */
static void infer_model_updated_cb(GstElement *gie, gint err, const gchar *config_file)
{
    double otaTime = 0;
...
void apply_ota(AppCtx *ota_appCtx)
{
    ...
    if (ota_appCtx->override_config.primary_gie_config.enable) {
        primary_gie = ota_appCtx->pipeline.common_elements.primary_gie_bin.primary_gie;
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < num_instances; i++) {
        ...
        if (!create_pipeline(appCtx[i], bbox_generated_probe_after_analytics, NULL, perf_cb,
                             overlay_graphics)) {
        ...
        g_object_set(appCtx[i]->pipeline.multi_src_bin.streammux, "buffer-pool-size",
                     STREAMMUX_BUFFER_POOL_SIZE, NULL);
    }

                                          ...
                                          const char *group)
{
    ...
    for (int i = 0; i < docs_indx_vec_size; i++) {
        ...
        for (YAML::const_iterator itr = docs[indx][group].begin(); itr != docs[indx][group].end();
             ++itr) {
            ...
            if (paramKey == "udpport") {
                ...
                g_object_set(G_OBJECT(server), "service", itr->second.as<std::string>().c_str(),
                             NULL);
                                         ...
                                         const char *group)
{
    ...
    for (int i = 0; i < docs_indx_vec_size; i++) {
        ...
        for (YAML::const_iterator itr = docs[indx][group].begin(); itr != docs[indx][group].end();
            ...
            if (paramKey == "bitrate") {
                g_object_set(G_OBJECT(encoder), "bitrate", itr->second.as<guint>(), NULL);
            } else if (paramKey == "iframeinterval") {
                g_object_set(G_OBJECT(encoder), "iframeinterval", itr->second.as<guint>(), NULL);
            } else {
                std::cerr << "!! [WARNING] Unknown param found : " << paramKey << std::endl;
            }
        }
                                                        ...
                                                        const char *group)
{
    ...
    if (docs["customlib-name"]) {
        ...
        g_object_set(G_OBJECT(vtemplate), "customlib-name", libPath.c_str(), NULL);
    }
    ...
    if (docs["customlib-props"]) {
        ...
        for (uint32_t i = 0; i < listNode.size(); i++) {
            ...
            g_object_set(G_OBJECT(vtemplate), "customlib-props", tmpProb.c_str(), NULL);
        }
    }
    ...
    for (int i = 0; i < docs_indx_vec_size; i++) {
        ...
        for (YAML::const_iterator itr = docs[indx][group].begin(); itr != docs[indx][group].end();
            ...
            if (paramKey == "enable" && docs[indx][group]["enable"].as<gboolean>() == TRUE) {
                ...
                std::cerr << "!! [WARNING] Unknown param found for nvdsanalytics: " << paramKey
                          << std::endl;
            }
        }
    }
...
static bool is_enc_hw_support()
{
    ...
    if (prop.integrated) {
        ...
        for (uint32_t i = 0; i < sizeof(dgpus_unsupport_hw_enc) / sizeof(dgpus_unsupport_hw_enc[0]);
             i++) {
        ...
        if (!docs[i][group].IsNull()) {
            if (docs[i][group]["enc-type"]) {
                ...
                if (val == 0 && !is_enc_hw_support()) {
                    g_print(
                        "** WARN: hardware encoding is not supported, fallback to software "
                        "encoding \n");
                    val = 1;
                }
                return val;
...
guint ds_parse_group_enable(gchar *cfg_file_path, const char *group)
{
    std::string paramKey = "";
    std::vector<YAML::Node> docs = YAML::LoadAllFromFile(cfg_file_path);
    std::vector<int> docs_indx_vec;
                                            ...
                                            const char *group)
{
    ...
    for (YAML::const_iterator itr = configyml[group].begin(); itr != configyml[group].end();
        ...
        if (paramKey == "config-file-path") {
            ...
            if (!get_absolute_file_path_yaml(app_cfg_file_path, str, path)) {
                ret = NVDS_YAML_PARSER_ERROR;
            }
            g_object_set(G_OBJECT(element), "postprocesslib-config-file", path, NULL);
            ...
            g_object_set(G_OBJECT(element), "postprocesslib-name", path, NULL);

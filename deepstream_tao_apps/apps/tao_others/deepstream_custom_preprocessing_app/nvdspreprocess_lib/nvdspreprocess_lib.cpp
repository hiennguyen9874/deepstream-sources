...
static std::pair<EglFrameCacheIter, bool> cache_egl_frame(CustomCtx *ctx,
...
static std::tuple<void *, void *> get_planes_addr(CustomCtx *ctx,
                                                  NvBufSurface *surface,
                                                  int batch_index)
{
    const NvBufSurfaceParams *surface_params = &surface->surfaceList[batch_index];
                                             ...
                                             NvDsPreProcessAcquirer *acquirer)
{
    ...
    ctx->conv_handle->run(input_format, output_dtype, output_format, ctx->interpolation,
                          ctx->stream, false);

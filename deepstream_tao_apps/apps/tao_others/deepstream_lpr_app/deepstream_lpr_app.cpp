...
static GstPadProbeReturn nvdsanalytics_src_pad_buffer_probe(GstPad *pad,
                                                            GstPadProbeInfo *info,
                                                            gpointer u_data)
{
    GstBuffer *buf = (GstBuffer *)info->data;
...
int main(int argc, char *argv[])
{
    ...
    GstElement *pipeline = NULL, *streammux = NULL, *sink = NULL, *primary_detector = NULL,
               *secondary_detector = NULL, *nvvidconv = NULL, *nvosd = NULL, *nvvidconv1 = NULL,
               *outenc = NULL, *capfilt = NULL, *mux = NULL, *secondary_classifier = NULL,
    ...
    GstElement *h264parser[128], *source[128], *decoder[128], *mp4demux[128], *parsequeue[128];
    GstBus *bus = NULL;
    guint bus_watch_id;
    ...
    RETURN_ON_PARSER_ERROR(nvds_parse_source_list(&g_list, argv[1], "source-list"));
    ...
    for (iterator = g_list, src_cnt = 0; iterator; iterator = iterator->next, src_cnt++) {
        ...
        gst_bin_add_many(GST_BIN(pipeline), source[src_cnt], mp4demux[src_cnt], h264parser[src_cnt],
                         parsequeue[src_cnt], decoder[src_cnt], NULL);
        ...
        if (!gst_element_link_pads(source[src_cnt], "src", mp4demux[src_cnt], "sink")) {
        ...
        if (!gst_element_link_many(h264parser[src_cnt], parsequeue[src_cnt], decoder[src_cnt],
                                   NULL)) {
            g_printerr("Elements could not be linked: 1. Exiting.\n");
        }

        /* we set the input filename to the source element */
        g_object_set(G_OBJECT(source[src_cnt]), "location", (gchar *)iterator->data, NULL);
    ...
    primary_detector = gst_element_factory_make(infer_plugin(pgie_type), "primary-infer-engine1");

    secondary_detector =
        gst_element_factory_make(infer_plugin(sgie0_type), "secondary-infer-engine1");

    secondary_classifier =
        gst_element_factory_make(infer_plugin(sgie1_type), "secondary-infer-engine2");
    ...
    if (!primary_detector || !secondary_detector || !nvvidconv || !nvosd || !sink) {
        g_printerr("One element could not be created. Exiting.\n");
        return -1;
    }
    ...
    gst_bin_add_many(GST_BIN(pipeline), primary_detector, secondary_detector, tracker,
                     nvdsanalytics, queue1, queue2, queue3, queue4, queue5, queue6, queue7, queue8,
                     secondary_classifier, nvvidconv, nvosd, nvtile, sink, NULL);

    if (!gst_element_link_many(streammux, queue1, primary_detector, queue2, tracker, queue3,
                               nvdsanalytics, queue4, secondary_detector, queue5,
    ...
    if (output_type == 1) {
        ...
        gst_bin_add_many(GST_BIN(pipeline), queue9, nvvidconv1, capfilt, queue10, outenc, encparse,
                         mux, sink, NULL);

        if (!gst_element_link_many(nvosd, queue9, nvvidconv1, capfilt, queue10, outenc, encparse,
                                   mux, sink, NULL)) {

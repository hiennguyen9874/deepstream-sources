...
static const bool ENABLE_TENSOR_DEBUG = []() {
    ...
    if (!debug_env) {
        std::cout << "[" << __FILE__ << "] "
                  << "Environment variable " << ENABLE_ENV_NAME
                  << " not set, DEBUG_DS_TENSOR disabled" << std::endl;
        return false;
    }
    ...
    std::cout << "[" << __FILE__ << "] "
              ...
              << ", DEBUG_DS_TENSOR " << (is_enabled ? "enabled" : "disabled") << std::endl;
    return is_enabled;
}();
 ...
 */
inline void GetBlockCoordinates(size_t block_idx,
 ...
 * It prints the first 5 blocks of the tensor to the console and then dumps the full tensor to a
 * file.
 *
 * @example
 ...
 */
template <typename T>
void DumpTensorData(const char *tensor_name,
                    ...
                    size_t last_dim_size)
{
    ...
    for (unsigned int i = 0; i < layer_ref.inferDims.numDims; i++) {
        ...
        if (i < layer_ref.inferDims.numDims - 1)
            outfile << ", ";
    }
    outfile << "]\n\n";
    ...
    outfile.close();
    std::cout << "[" << GetTimestamp() << "] DEBUG_DS_TENSOR: Full tensor dumped to " << filename
              << std::endl;
}
 ...
 * It validates the tensor, prints the dimensions, and then prints the tensor data based on the data
 * type.
 */
 ...
 * It prints the first 100 values of the tensor to the console and then dumps the full tensor to a
 * file.
 */
#define DEBUG_TENSOR(tensor_name, data, size, type)                                        \
    if (ENABLE_TENSOR_DEBUG) {                                                             \
        ...
        if (outfile.is_open()) {                                                           \
            ...
            outfile.close();                                                               \
            std::cout << "[" << GetTimestamp() << "] DEBUG_TENSOR: Full tensor dumped to " \
                      << filename << std::endl;                                            \
        }                                                                                  \
    }

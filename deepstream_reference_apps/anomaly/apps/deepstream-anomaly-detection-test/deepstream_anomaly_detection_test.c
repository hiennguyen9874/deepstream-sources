...
{
    GMainLoop *loop = NULL;
    GstElement *pipeline = NULL, *streammux = NULL, *streammux_queue = NULL, *sink_of = NULL,
               *pgie_queue = NULL, *dsdirection_queue = NULL, *nvvidconv_queue = NULL,
               *nvosd_queue = NULL, *tiler_infer_queue = NULL, *tiler_of = NULL, *nvof = NULL,
    ...
    tiler_infer = gst_element_factory_make("nvmultistreamtiler", "nvtiler-infer");
    tiler_infer_queue = gst_element_factory_make("queue", "nvtiler-infer-queue");
    ...
    g_object_set(G_OBJECT(pgie), "config-file-path", "dsanomaly_pgie_config.txt", NULL);
    ...
    g_object_set(G_OBJECT(tiler_of), "rows", tiler_rows, "columns", tiler_columns, "width",
                 TILED_OUTPUT_WIDTH_OF, "height", TILED_OUTPUT_HEIGHT_OF, NULL);
    g_object_set(G_OBJECT(tiler_infer), "rows", tiler_rows, "columns", tiler_columns, "width",
                 TILED_OUTPUT_WIDTH_INFER, "height", TILED_OUTPUT_HEIGHT_INFER, NULL);

    /* We set the sink properties here */
    g_object_set(G_OBJECT(sink_of), "window-x", 0, "window-y", 0, NULL);
    g_object_set(G_OBJECT(sink_infer), "window-x", TILED_OUTPUT_WIDTH_OF, "window-y",
    ...
    gst_bin_add_many(GST_BIN(pipeline), streammux_queue, pgie, pgie_queue, nvof, of_queue,
                     dsdirection, dsdirection_queue, tee, of_branch_queue, nvofvisual,
                     ofvisual_queue, tiler_of, sink_of, infer_branch_queue, tiler_infer,

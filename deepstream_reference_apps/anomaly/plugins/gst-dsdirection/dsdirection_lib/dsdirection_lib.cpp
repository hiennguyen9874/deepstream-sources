...
{
    DsDirectionOutput *out = (DsDirectionOutput *)calloc(1, sizeof(DsDirectionOutput));
    ...
    for (unsigned int j = rect_param->top; j <= rect_param->top + rect_param->height; j++) {
        unsigned int block_j = j / flow_bsize;
        for (unsigned int i = rect_param->left; i <= rect_param->left + rect_param->width; i++) {

...
enum { PROP_0, PROP_UNIQUE_ID };
// Block size used in NVOF application
#define NVOF_BLK_SIZE 4
/*set the user metadata type*/
#define NVDS_DIRECTION_USER_META \
    (nvds_get_user_meta_type(((gchar *)"NVIDIA.NVDSDIRECTION.DIR_META")))
...
static GstStaticPadTemplate gst_dsdirection_sink_template = GST_STATIC_PAD_TEMPLATE(
    "sink",
    GST_PAD_SINK,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS(
...
static GstStaticPadTemplate gst_dsdirection_src_template = GST_STATIC_PAD_TEMPLATE(
    "src",
    GST_PAD_SRC,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS(
...
static void gst_dsdirection_set_property(GObject *object,
                                         guint prop_id,
                                         const GValue *value,
                                         GParamSpec *pspec);
static void gst_dsdirection_get_property(GObject *object,
                                         guint prop_id,
                                         GValue *value,
                                         GParamSpec *pspec);

static GstFlowReturn gst_dsdirection_transform_ip(GstBaseTransform *btrans, GstBuffer *inbuf);
 ...
 * element.
 */
static void gst_dsdirection_class_init(GstDsDirectionClass *klass)
{
    ...
    gobject_class->set_property = GST_DEBUG_FUNCPTR(gst_dsdirection_set_property);
    gobject_class->get_property = GST_DEBUG_FUNCPTR(gst_dsdirection_get_property);

    gstbasetransform_class->transform_ip = GST_DEBUG_FUNCPTR(gst_dsdirection_transform_ip);
    ...
    gst_element_class_set_details_simple(
        gstelement_class, "DsDirection plugin", "DsDirection Plugin",
        "Estimate direction in which object is moving",
        "NVIDIA Corporation. Post on Deepstream for Tesla forum for any queries "
        "@ https://devtalk.nvidia.com/default/board/209/");
}

static void gst_dsdirection_init(GstDsDirection *dsdirection)
{
...
static GstFlowReturn gst_dsdirection_transform_ip(GstBaseTransform *btrans, GstBuffer *inbuf)
{
    ...
    GST_DEBUG_OBJECT(dsdirection, "Processing Frame %lu", dsdirection->frame_num);
            ...
            if (of_user_meta && of_user_meta->base_meta.meta_type == NVDS_OPTICAL_FLOW_META) {
                ...
                if (ofmeta) {
                    ...
                    for (l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
                        ...
                        output =
                            DsDirectionProcess((NvOFFlowVector *)ofmeta->data, ofmeta->cols,
                                               ofmeta->rows, NVOF_BLK_SIZE, &obj_meta->rect_params);
...
static gpointer copy_ds_direction_meta(gpointer data, gpointer user_data)
{
    NvDsUserMeta *user_meta = (NvDsUserMeta *)data;
    DsDirectionOutput *src_user_metadata = (DsDirectionOutput *)user_meta->user_meta_data;
...
static void release_ds_direction_meta(gpointer data, gpointer user_data)
{
    NvDsUserMeta *user_meta = (NvDsUserMeta *)data;
                                   ...
                                   DsDirectionOutput *output)
{
    ...
    if (text_params.display_text) {
        gchar *conc_string =
            g_strconcat(text_params.display_text, " ", output->object.direction, NULL);
 ...
 */
static gboolean dsdirection_plugin_init(GstPlugin *plugin)
{
    GST_DEBUG_CATEGORY_INIT(gst_dsdirection_debug, "dsdirection", 0, "dsdirection plugin");

    return gst_element_register(plugin, "dsdirection", GST_RANK_PRIMARY, GST_TYPE_DSDIRECTION);
}

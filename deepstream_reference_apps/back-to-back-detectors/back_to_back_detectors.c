...
int main(int argc, char *argv[])
{
    ...
    if (prop.integrated) {
        gst_bin_add_many(GST_BIN(pipeline), source, h264parser, decoder, streammux,
                         primary_detector, secondary_detector, nvvidconv, nvosd, transform, sink,
                         NULL);
    } else {
        gst_bin_add_many(GST_BIN(pipeline), source, h264parser, decoder, streammux,
                         primary_detector, secondary_detector, nvvidconv, nvosd, sink, NULL);
    }
    ...
    if (prop.integrated) {
        if (!gst_element_link_many(streammux, primary_detector, secondary_detector, nvvidconv,

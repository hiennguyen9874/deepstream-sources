                                                      ...
                                                      gpointer u_data)
{
    ...
    if (nvds_enable_latency_measurement) {
        ...
        latency_info =
            (NvDsFrameLatencyInfo *)calloc(1, ctx->num_sources * sizeof(NvDsFrameLatencyInfo));
        ;
        g_print("\n************BATCH-NUM = %d soure %d**************\n", batch_num,
                ctx->num_sources);
...
std::tuple<Vec2D<int>, Vec3D<float>> parse_objects_from_tensor_meta(
    NvDsInferTensorMeta *tensor_meta)
{
    ...
    Vec3D<float> score_graph =
        paf_score_graph(paf_data, paf_dims, topology, counts, refined_peaks, num_integral_samples);
    /* Assign weights to all edges in the bipartite graph generated */
    Vec3D<int> connections =
        assignment(score_graph, topology, counts, link_threshold, max_num_parts);
...
static void create_display_meta(Vec2D<int> &objects,
...
static GstPadProbeReturn body_pose_gie_src_pad_buffer_probe(GstPad *pad,
                                                            GstPadProbeInfo *info,
                                                            gpointer u_data)
{
    gchar *msg = NULL;
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next) {
        ...
        for (l_user = frame_meta->frame_user_meta_list; l_user != NULL; l_user = l_user->next) {
            ...
            if (user_meta->base_meta.meta_type == NVDSINFER_TENSOR_OUTPUT_META) {
                ...
                create_display_meta(objects, normalized_peaks, frame_meta,
...
static GstPadProbeReturn yolov4_gie_src_pad_buffer_probe(GstPad *pad,
                                                         GstPadProbeInfo *info,
                                                         gpointer u_data)
{
    gchar *msg = NULL;
...
gboolean link_element_to_metamux_sink_pad(GstElement *metamux, GstElement *elem, gint index)
{
    gboolean ret = FALSE;
...
gboolean unlink_element_from_metamux_sink_pad(GstElement *metamux, GstElement *elem)
{
    gboolean ret = FALSE;
    GstPad *mux_sink_pad = NULL;
    GstPad *src_pad = NULL;
...
gboolean link_streamdemux_to_streammux(NvDsParallelGieBin *bin,
                                       GstElement *demux,
                                       GstElement *mux,
                                       gint index)
{
...
gboolean create_primary_gie_videotemplate_bin(NvDsVideoTemplateConfig *config,
                                              NvDsPrimaryGieBin *bin)
{
    ...
    g_object_set(G_OBJECT(bin->primary_gie), "customlib-name", config->customlib_name, NULL);

    for (i = 0; i < config->num_customlib_props; i++) {
        g_object_set(G_OBJECT(bin->primary_gie), "customlib-props", config->customlib_props[i],
                     NULL);
    }
...
static gboolean create_parallel_infer_bin(guint num_sub_bins,
                                          NvDsConfig *config,
                                          NvDsParallelGieBin *bin,
                                          AppCtx *appCtx)
{
    gboolean ret = FALSE;
    GstElement *sink_elem = NULL;
    ...
    NVGSTDS_ELEM_ADD_PROBE(bin->muxer_buffer_probe_id, bin->muxer, "src",
                           body_pose_gie_src_pad_buffer_probe, GST_PAD_PROBE_TYPE_BUFFER, appCtx);
    ...
    bin->demuxer = gst_element_factory_make(NVDS_ELEM_STREAM_DEMUX, NULL);
    if (!bin->demuxer) {
    ...
    for (i = 0; i < num_sub_bins; i++) {
        ...
        if (config->tracker_config[i].enable && config->dsanalytics_config[i].enable) {
            if (!create_dsanalytics_bin(&config->dsanalytics_config[i], &bin->dsanalytics_bin[i])) {
        ...
        if (config->tracker_config[i].enable) {
            if (!create_tracking_bin(&config->tracker_config[i], &bin->tracker_bin[i])) {
            ...
            gst_bin_add(GST_BIN(bin->bin), bin->tracker_bin[i].bin);
        ...
        bin->streammux[i] = gst_element_factory_make(NVDS_ELEM_STREAM_MUX, NULL);
        if (!bin->streammux[i]) {
        ...
        if (config->streammux_config.is_parsed) {
            if (!set_streammux_properties(&config->streammux_config, bin->streammux[i])) {
...
static gboolean add_and_link_broker_sink(AppCtx *appCtx)
{
    ...
    for (guint i = 0; i < config->num_sink_sub_bins; i++) {
        if (config->sink_bin_sub_bin_config[i].type == NV_DS_SINK_MSG_CONV_BROKER) {
            ...
            if (!link_element_to_tee_src_pad(instance_bin->sink_tee,
...
int main(int argc, char *argv[])
{
    ...
    if (use_new_mux) {
        ...
        for (int i = 0; i < config->num_source_sub_bins; i++) {
            g_object_get(G_OBJECT(pipeline->multi_src_bin.sub_bins[i].cap_filter1), "caps", &caps,
                         NULL);
            caps_string = gst_caps_to_string(caps);
            snprintf(strCaps, MAX_STR_LEN - 1, "%s,width=%d, height=%d", caps_string,
                     config->streammux_config.pipeline_width,
                     config->streammux_config.pipeline_height);
            fiter_caps = gst_caps_from_string(strCaps);
            g_object_set(G_OBJECT(pipeline->multi_src_bin.sub_bins[i].cap_filter1), "caps",
                         fiter_caps, NULL);
            printf("strCaps:%s\n", strCaps);
    ...
    if (!create_parallel_infer_bin(config->num_primary_gie_sub_bins, config,
                                   &pipeline->parallel_infer_bin, appCtx)) {
    ...
    if (config->msg_conv_config.enable) {
        ...
        g_object_set(
            G_OBJECT(instance_bin->msg_conv), "config", convConfig->config_file_path, "msg2p-lib",
            (convConfig->conv_msg2p_lib ? convConfig->conv_msg2p_lib : "null"), "payload-type",
            convConfig->conv_payload_type, "comp-id", convConfig->conv_comp_id, "debug-payload-dir",
            convConfig->debug_payload_dir, "multiple-payloads", convConfig->multiple_payloads,
            "msg2p-newapi", convConfig->conv_msg2p_new_api, "frame-interval",
            convConfig->conv_frame_interval, NULL);

        gst_bin_add(GST_BIN(pipeline->pipeline), instance_bin->msg_conv);
    ...
    if (config->tiled_display_config.enable) {
        ...
        if (config->show_source != -1) {
            ...
            g_object_set(G_OBJECT(pipeline->tiled_display_bin.tiler), "show-source",
                         config->show_source, NULL);
    ...
    if (config->osd_config.enable) {
        ...
        if (!osd_sink_pad)
            ...
            gst_pad_add_probe(osd_sink_pad, GST_PAD_PROBE_TYPE_BUFFER, latency_measurement_buf_prob,
                              ctx, NULL);
        }
    }
    ...
    // x264enc will output one buffer after input 66 buffers at default, enable zerolatency
    // property.

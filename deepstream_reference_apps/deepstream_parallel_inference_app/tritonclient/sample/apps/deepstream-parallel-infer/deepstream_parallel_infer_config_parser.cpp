...
gboolean parse_multi_preprocess_yaml(NvDsPreProcessConfig *config,
                                     std::string group,
                                     gchar *cfg_file_path)
{
    gboolean ret = FALSE;
...
gboolean parse_video_template_yaml(NvDsVideoTemplateConfig *config,
                                   std::string group,
                                   gchar *cfg_file_path)
{
    gboolean ret = FALSE;
    ...
    for (YAML::const_iterator itr = configyml[group].begin(); itr != configyml[group].end();
        ...
        if (paramKey == "enable") {
            ...
            std::string temp = itr->second.as<std::string>();
            config->customlib_props[config->num_customlib_props] =
...
static gboolean parse_sgie_yaml(NvDsConfig *config,
                                NvDsGieConfig *gieConfig,
                                std::string group,
                                gchar *cfg_file_path,
                                gboolean enable)
{
    ...
    for (YAML::const_iterator itr = configyml.begin(); itr != configyml.end(); ++itr) {
        std::string paramKey = itr->first.as<std::string>();
        parse_err = !parse_gie_yaml(gieConfig + sgie_num, paramKey, cfg_file_path);
...
gboolean parse_tiled_display_yaml(NvDsConfig *appConfig,
...
gboolean parse_config_file_yaml(NvDsConfig *config, gchar *cfg_file_path)
{
    ...
    for (YAML::const_iterator itr = configyml.begin(); itr != configyml.end(); ++itr) {
        ...
        if (paramKey == "application") {
            ...
            parse_err = !parse_multi_preprocess_yaml(
                &config->pre_process_sub_bin_config[config->num_pre_process_sub_bins], paramKey,
                cfg_file_path);
            ...
            parse_err = !parse_gie_yaml(
                &config->primary_gie_sub_bin_config[config->num_primary_gie_sub_bins], paramKey,
                cfg_file_path);
            ...
            parse_err = !parse_video_template_yaml(
                &config->video_template_sub_bin_config[config->num_primary_gie_sub_bins], paramKey,
                cfg_file_path);
            ...
            if (configyml[paramKey]["pgie-id"].as<int>()) {
                ...
                if (configyml[paramKey]["src-ids"]) {
                    ...
                    config->srcids_config[index].src_ids =
                        (char *)calloc(sizeof(char) * src_ids.size(), sizeof(char));
                    std::strncpy(config->srcids_config[index].src_ids, src_ids.c_str(),
                                 src_ids.size());
                    g_print("src_ids:%s\n", config->srcids_config[index].src_ids);
                }
            }
            ...
            if (configyml[paramKey]["cfg-file-path"]) {
                ...
                parse_err = !parse_tracker_yaml(
                    &config->tracker_config[config->num_tracker_sub_bins], abs_csv_path);
                g_free(abs_csv_path);
            ...
            if (configyml[paramKey]["cfg-file-path"]) {
                ...
                parse_err = !parse_sgie_yaml(config, pCfg, paramKey, abs_csv_path, enable);
            }
            ...
            parse_err =
                !parse_tiled_display_yaml(config, &config->tiled_display_config, cfg_file_path);
            ...
            if (configyml[paramKey]["cfg-file-path"]) {
                ...
                if (!get_absolute_file_path_yaml(
                        cfg_file_path, str,
                        config->dsanalytics_config[config->num_analysis_sub_bins]
                            .config_file_path)) {

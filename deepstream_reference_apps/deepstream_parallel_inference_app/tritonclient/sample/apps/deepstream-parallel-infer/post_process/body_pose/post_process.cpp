...
static Vec2D<int> topology{{0, 1, 15, 13},  {2, 3, 13, 11},  {4, 5, 16, 14},  {6, 7, 14, 12},
                           {8, 9, 11, 12},  {10, 11, 5, 7},  {12, 13, 6, 8},  {14, 15, 7, 9},
                           {16, 17, 8, 10}, {18, 19, 1, 2},  {20, 21, 0, 1},  {22, 23, 0, 2},
                           {24, 25, 1, 3},  {26, 27, 2, 4},  {28, 29, 3, 5},  {30, 31, 4, 6},
                           {32, 33, 17, 0}, {34, 35, 17, 5}, {36, 37, 17, 6}, {38, 39, 17, 11},
                           {40, 41, 17, 12}};

/* Method to find peaks in the output tensor. 'window_size' represents how many pixels we are
   considering at once to find a maximum value, or a ‘peak’. Once we find a peak, we mark it using
   the ‘is_peak’ boolean in the inner loop and assign this maximum value to the center pixel of our
   window. This is then repeated until we cover the entire frame. */
void find_peaks(Vec1D<int> &counts_out,
                Vec3D<int> &peaks_out,
                void *cmap_data,
                NvDsInferDims &cmap_dims,
                float threshold,
                int window_size,
                int max_count)
{
...
Vec3D<float> refine_peaks(Vec1D<int> &counts,
                          Vec3D<int> &peaks,
                          void *cmap_data,
                          NvDsInferDims &cmap_dims,
                          int window_size)
{
    ...
    Vec3D<float> refined_peaks(peaks.size(),
                               Vec2D<float>(peaks[0].size(), Vec1D<float>(peaks[0][0].size(), 0)));
...
/* Create a bipartite graph to assign detected body-parts to a unique person in the frame. This
   method also takes care of finding the line integral to assign scores to these points */
Vec3D<float> paf_score_graph(void *paf_data,
                             NvDsInferDims &paf_dims,
                             Vec2D<int> &topology,
                             Vec1D<int> &counts,
                             Vec3D<float> &peaks,
                             int num_integral_samples)
{
    int K = topology.size();
    int H = paf_dims.d[1];
    int W = paf_dims.d[2];
...
/*
 This method takes care of solving the graph assignment problem using Munkres algorithm. Munkres
 algorithm is defind in 'munkres_algorithm.cpp'
 */

Vec3D<int> assignment(Vec3D<float> &score_graph,
                      Vec2D<int> &topology,
                      Vec1D<int> &counts,
...
Vec2D<int> connect_parts(Vec3D<int> &connections,
                         Vec2D<int> &topology,
                         Vec1D<int> &counts,
                         int max_count)
{

 ...
 */
static GstPadProbeReturn sink_input_buffer_callback(GstPad *pad,
...
static gboolean bus_call(GstBus *bus, GstMessage *msg, gpointer data)
{
    ...
    case GST_MESSAGE_APPLICATION: {
        ...
        if (gst_structure_has_name(str, "dynamic-src-bin-file-change")) {
            GstElement *dynamicsrcbin =
                get_element_by_factory_name(GST_BIN(app->pipeline), "nvdsdynamicsrcbin");
            if (!dynamicsrcbin) {
...
int main(int argc, char *argv[])
{
    ...
    gst_pad_add_probe(sink_pad, GST_PAD_PROBE_TYPE_BUFFER | GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM,
                      (GstPadProbeCallback)sink_input_buffer_callback, NULL, NULL);

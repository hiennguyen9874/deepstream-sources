...
static GstPadProbeReturn server_sink_sink_pad_buffer_probe(GstPad *pad,
                                                           ...
                                                           gpointer u_data)
{
    if (g_perf_mode) {
        ...
        if (gst_buffer_is_writable(buf)) {
            GstCaps *caps = gst_caps_new_simple("video/x-raw", "server_send_time", G_TYPE_DOUBLE,
                                                get_current_timestamp(), NULL);
            gst_buffer_add_reference_timestamp_meta(buf, caps, 0, 0);
            gst_caps_unref(caps);
            // g_print ("server_send_time %.3f %s\n", get_current_timestamp(), gst_caps_to_string
            // (caps));
        }
    }
...
static GstPadProbeReturn client_source_src_pad_buffer_probe(GstPad *pad,
                                                            GstPadProbeInfo *info,
                                                            gpointer u_data)
{
    if (g_perf_mode) {
...
static GstPadProbeReturn client_sgie_src_pad_buffer_probe(GstPad *pad,
                                                          GstPadProbeInfo *info,
                                                          gpointer u_data)
{
    GstBuffer *buf = (GstBuffer *)info->data;
...
static GstPadProbeReturn client_osd_src_pad_buffer_probe(GstPad *pad,
                                                         GstPadProbeInfo *info,
                                                         gpointer u_data)
{
...
static int create_client_pipeline(int argc, char *argv[])
{
    GMainLoop *loop = NULL;
    GstElement *pipeline = NULL, *streammux = NULL, *sink = NULL, *pgie = NULL, *sgie = NULL,
               *sr_conv = NULL, *sr_capsfilter = NULL, *sr_video_template = NULL, *queue1, *queue2,
    ...
    g_object_set(G_OBJECT(sr_video_template), "customlib-name",
                 "./video_template_impl/libnvds_vt_impl.so", NULL);
    g_object_set(G_OBJECT(sr_video_template), "customlib-props",
                 "config-file:config_videotemplate.yml", NULL);
    ...
    g_object_set(G_OBJECT(pgie), "config-file-path", "dsipctest_pgie_config.yml",
                 "output-tensor-meta", TRUE, NULL);
    g_object_set(G_OBJECT(sgie), "config-file-path", "dsipctest_sgie_config.yml", NULL);
    ...
    gst_bin_add_many(GST_BIN(pipeline), queue1, pgie, sr_conv, sr_capsfilter, sr_video_template,
    ...
    if (!gst_element_link_many(streammux, queue1, pgie, sr_conv, sr_capsfilter, sr_video_template,
                               queue2, sgie, tiler, queue3, nvvidconv, queue4, nvosd, queue5, sink,
                               NULL)) {
        g_printerr("Elements could not be linked. Exiting.\n");
        return -1;
    }
    ...
    if (!src_pad)
        ...
        gst_pad_add_probe(src_pad, GST_PAD_PROBE_TYPE_BUFFER, client_sgie_src_pad_buffer_probe,
                          NULL, NULL);
    gst_object_unref(src_pad);
    ...
    if (!src_pad)
        ...
        gst_pad_add_probe(src_pad, GST_PAD_PROBE_TYPE_BUFFER, client_osd_src_pad_buffer_probe,
                          (gpointer)&num_sources, NULL);
    gst_object_unref(src_pad);
...
static int create_server_pipeline(int argc, char *argv[])
{
    guint i = 0, num_sources = 0;
    ...
    for (i = 0; i < num_sources; i++) {
        ...
        g_print("server is started uri: %s path: %s\n", gAppCtx.ipcserver[i].uri,
                gAppCtx.ipcserver[i].socket_path);
        g_object_set(G_OBJECT(sink), "socket-path", gAppCtx.ipcserver[i].socket_path,
                     "buffer_timestamp_copy", TRUE, NULL);
        if (g_perf_mode) {
            g_object_set(G_OBJECT(sink), "sync", FALSE, "meta-serialization-lib",
                         "latency_serialization/liblatency_serialization.so", NULL);
        }
...
int main(int argc, char *argv[])
{
    ...
    g_perf_mode = g_getenv("IPC_SR_PERF_MODE") && !g_strcmp0(g_getenv("IPC_SR_PERF_MODE"), "1");

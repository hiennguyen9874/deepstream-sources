...
GOptionEntry entries[] = {
    ...
    {"input-file", 'i', 0, G_OPTION_ARG_FILENAME, &input_file, "Set the input H264 file", NULL},
    ...
    for (l_frame = batch_meta->frame_meta_list; l_frame; l_frame = l_frame->next) {
        ...
        for (l_obj = frame_meta->obj_meta_list; l_obj; l_obj = l_obj->next) {
            ...
            g_snprintf(txt_params->display_text, MAX_DISPLAY_LEN, "%s %ld %s",
                       pgie_classes_str[obj_meta->class_id], obj_meta->object_id,
                       sgie_label); /* Person 12 hasBasket */
...
static void check_gst_element_creation_success(GstElement *element, gchar *component_name)
{
    if (!element) {
...
int main(int argc, char *argv[])
{
    ...
    GstElement *pipeline = NULL, *source = NULL, *h264parser = NULL, *decoder = NULL, *sink = NULL,
               *pgie = NULL, *sgie = NULL, *nvvidconv = NULL, *nvosd = NULL, *nvstreammux = NULL,
               *nvtracker = NULL;
    ...
    if (!pipeline || !source || !h264parser || !decoder || !nvstreammux || !pgie || !sgie ||
        !nvtracker || !nvvidconv || !nvosd || !msgconv || !msgbroker || !tee || !queue1 ||
        !queue2 || !sink) {
    ...
    if (argc > 1 && IS_YAML(argv[1])) {
        ...
        g_object_set(G_OBJECT(pgie), "config-file-path", "configs/pgie_config_peoplenet.yml", NULL);

        g_object_set(G_OBJECT(sgie), "config-file-path", "configs/basket_classifier.yml", NULL);

        g_object_set(G_OBJECT(msgconv), "config", "configs/dstest4_msgconv_config.yml", NULL);
    ...
    gst_bin_add_many(GST_BIN(pipeline), source, h264parser, decoder, nvstreammux, pgie, sgie,
                     nvtracker, nvvidconv, nvosd, tee, queue1, queue2, msgconv, msgbroker, sink,
                     NULL);

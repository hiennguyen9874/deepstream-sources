                                    ...
                                    NvDsFrameMeta *frame_meta)
{
    ...
    //    g_print("source id: %d, Enter: %d, Exit: %d\n", meta->source_id, meta->lccum_cnt_entry,
    //    meta->lccum_cnt_exit);
}
        ...
        for (l = frame_meta->obj_meta_list; l != NULL; l = l->next) {
...

            {
                ...
                generate_event_msg_meta(
                    ...
                    appCtx->config.multi_source_config[stream_id].camera_id, user_data, scaleW,
                    scaleH, frame_meta);
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < num_instances; i++) {
        ...
        /*  if (appCtx[i]->config.dsanalytics_config.enable){
           ...
           appCtx[i]->pipeline.common_elements.dsanalytics_bin.elem_dsanalytics; src_pad =
           gst_element_get_static_pad (nvdsanalytics, "src"); if (!src_pad) g_print ("Unable to get
           nvdsanalytics src pad\n"); else
            {

...
static JsonObject *generate_object_object(void *privData, NvDsEventMsgMeta *meta)
{
    ...
    case (NvDsObjectType)NVDS_OBJECT_TYPE_PERSON_EXT_POSE: {
        if (meta->extMsgSize) {
            ...
            // // DEBUG
            // g_message("generate_object_object(): pose_meta->num_poses = %d",
            // pose_meta->num_poses);
            ...
            for (int i = 0; i < pose_meta->num_poses; i++) {
                ...
                json_object_set_int_member(jobject, "bottomrightx",
                                           meta->bbox.left + meta->bbox.width);
                json_object_set_int_member(jobject, "bottomrighty",
                ...
                for (joint_index = 0; joint_index < pose_meta->poses[i].num_joints; joint_index++) {
                    if (pose_meta->poses[i].joints[joint_index].confidence > 0.0) {
                        ...
                        if (pose_meta->poses[i].pose_type == 0) {
                            json_object_set_int_member(jobject, "x",
                                                       pose_meta->poses[i].joints[joint_index].x);
                            json_object_set_int_member(jobject, "y",
                                                       pose_meta->poses[i].joints[joint_index].y);
                            json_object_set_int_member(
                                jobject, "confidence",
                                pose_meta->poses[i].joints[joint_index].confidence);
                            json_object_set_object_member(
                                pobject, reinterpret_cast<const gchar *>(json_name), jobject);
                        } else if ((pose_meta->poses[i].pose_type == 1) ||
                                   (pose_meta->poses[i].pose_type == 2)) {
                            // pose3d or pose25d from BodyPose3DNet
                            json_object_set_int_member(jobject, "x",
                                                       pose_meta->poses[i].joints[joint_index].x);
                            json_object_set_int_member(jobject, "y",
                                                       pose_meta->poses[i].joints[joint_index].y);
                            json_object_set_int_member(jobject, "z",
                                                       pose_meta->poses[i].joints[joint_index].z);
                            json_object_set_int_member(
                                jobject, "confidence",
                                pose_meta->poses[i].joints[joint_index].confidence);
                            json_object_set_object_member(
            ...
            case (NvDsObjectType)NVDS_OBJECT_TYPE_PERSON_EXT_POSE: {
                ...
                if (pose_meta) {
                    ...
                    for (int i = 0; i < pose_meta->num_poses; i++) {
                        ...
                        if (type == 0) {
                            ss << "|#|pose2D|";
                            for (int joint_index = 0; joint_index < pose_meta->poses[i].num_joints;
                                 joint_index++) {
                                std::string s =
                                    _joint_maps[type][joint_index] + "," +

    ...
    try {
        ...
        if (std::equal(suffix.rbegin(), suffix.rend(), file.rbegin())) {
            ...
            if (output_tensor_meta) {
                ...
                pipeline["nvstreammux"]
                    .getProperty("width", stream_width)
                    .getProperty("height", stream_height);
                // disable metadata generation from the built-in postprocessing
                pipeline["infer"].set("filter-out-class-ids", "0;1;2;3");
                pipeline.attach("infer", "resnet_tensor_parser", "tensor parser", "",
                                "stream-width", (int)stream_width, "stream-height",
                                (int)stream_height);
            } else {

...
public:
    ...
    ErrCode process_i(const struct VideoBridge2dInput *inputData,
                      ...
                      const abiOnBridgeDataCB *inputDataConsumedCb) final
    {
        ...
        OnGuardBridgeDataCBImpl consumedCbImpl =
            [gConsumedCb = std::move(guardConsumedCb), this](
                ErrCode code, const struct VideoBridge2dInput *data) { gConsumedCb(code, data); };
...
protected:
    ...
    virtual ErrCode processImpl(const struct VideoBridge2dInput *inputData,

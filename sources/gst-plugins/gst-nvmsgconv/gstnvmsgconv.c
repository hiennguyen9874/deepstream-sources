...
void gst_nvmsgconv_set_property(GObject *object, guint property_id,
...
void gst_nvmsgconv_get_property(GObject *object, guint property_id,
...
gst_nvmsgconv_start(GstBaseTransform *trans) {
    ...
    if (self->debugPayloadDir) {
        if (access(self->debugPayloadDir, F_OK)) {
            GST_ELEMENT_ERROR(self, RESOURCE, NOT_FOUND, ("Error: dir %s not found", self->debugPayloadDir), (NULL));
            return FALSE;
        }
        if (access(self->debugPayloadDir, W_OK)) {
            GST_ELEMENT_ERROR(self, RESOURCE, WRITE, ("Error: User doesn't have write permission in %s", self->debugPayloadDir), (NULL));
            return FALSE;
        }
    }

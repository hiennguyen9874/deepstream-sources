...
static NvDsMsgConvInputType gst_nvmsgconv_get_input_type(GstCaps *caps)
{
    ...
    const int mtLen = strlen(mimetype);
...
static gpointer gst_nvmsgconv_copy_meta(gpointer data, gpointer uData)
{
    ...
    if (srcPayload) {
        ...
        outPayload->payload = g_memdup2(srcPayload->payload, srcPayload->payloadSize);
...
static void gst_nvmsgconv_class_init(GstNvMsgConvClass *klass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_enum(
            "payload-type", "Payload type", "Type of payload to be generated",
    ...
    g_object_class_install_property(
        gobject_class, PROP_DUMMY_PAYLOAD,
        g_param_spec_boolean(
            "dummy-payload", "Dummy payload",
...
static int add_payload_to_batch_meta(GstNvMsgConv *self,
...
static GstFlowReturn gst_nvmsgconv_transform_ip_ds3d(GstBaseTransform *trans, GstBuffer *buffer)
{
    GstNvMsgConv *self = GST_NVMSGCONV(trans);
    ...
    if (!isGood(NvDs3D_Find1stDataMap(buffer, refDataMap))) {
        GST_ERROR_OBJECT(self, "gst buffer doesn have ds3d datamap, will support later");
    ...
    if ((self->payloadType == NVDS_PAYLOAD_DEEPSTREAM_MINIMAL) ||
        ...
        int errcode =
            add_payload_to_batch_meta(self, payloads, payloadCount, batch_meta, NULL, NULL);
...
static void generate_ts_from_frame_ntp(char *buf, int buf_size, GstClockTime ts)
{
    time_t tloc;
    struct tm tm_log;
    char strmsec[6]; //.nnnZ\0
...
static GstFlowReturn gst_nvmsgconv_transform_ip_sparse4d(GstBaseTransform *trans, GstBuffer *buffer)
{
    GstNvMsgConv *self = GST_NVMSGCONV(trans);
    NvDsPayload **payloads = NULL;
    NvDsMetaList *l = NULL;
    ...
    if (batch_meta) {
        if ((self->payloadType == NVDS_PAYLOAD_DEEPSTREAM_MINIMAL) ||
            ...
            for (l = batch_meta->batch_user_meta_list; l; l = l->next) {
                ...
                if (user_event_meta &&
                    ...
                    for (guint i = 0; i < bbox3d_list->num_objects; i++) {
                        ...
                        eventMsg->type =
                            NVDS_EVENT_ENTRY; // Assuming this is an object detection event
                        eventMsg->frameId = bbox3d_list->frame_num;
                        eventMsg->sensorId = bbox3d_list->camera_id;
                        memcpy(eventMsg->entries, bbox3d_list->entries,
                               sizeof(SourceTimePair) * bbox3d_list->count);
                        eventMsg->count = bbox3d_list->count;
                        eventMsg->ts = (gchar *)g_malloc0(MAX_TIME_STAMP_LEN + 1);
                        nanoseconds_to_rfc3339(bbox3d_list->timestamp, eventMsg->ts,
                                               MAX_TIME_STAMP_LEN);
            ...
            if (self->multiplePayloads)
                payloads =
                    self->msg2p_generate_multiple(self->pCtx, eventList, eventCount, &payloadCount);
            else {
                payloads = (NvDsPayload **)g_malloc0(sizeof(NvDsPayload *) * 1);
...
static GstFlowReturn gst_nvmsgconv_transform_ip_video_audio(GstBaseTransform *trans, GstBuffer *buf)
{
    GstNvMsgConv *self = GST_NVMSGCONV(trans);
    NvDsPayload **payloads = NULL;
                ...
                if ((self->payloadType == NVDS_PAYLOAD_DEEPSTREAM_MINIMAL) ||
                    ...
                    if (eventCount || self->dummyPayload) {
                        ...
                        if (!eventCount || self->dummyPayload) {
                            ...
                            generate_ts_from_frame_ntp(
                                msg_meta->ts, MAX_TIME_STAMP_LEN,

                              ...
                              GstNvInferColorParams &color_params)
{
    ...
    for (key = keys; *key; key++)
    {
        ...
        else if (!g_strcmp0(*key, CONFIG_GROUP_INFER_CLASS_ATTRS_BORDER_COLOR))
        {
            ...
            if (length != 4)
            {
                g_printerr("Error: Group %s, Number of Color params should be exactly 4 "
                        ...
                        GKeyFile *key_file, const gchar *cfg_file_path)
{
    ...
    for (key = keys; *key; key++)
    {
        ...
        else if (!g_strcmp0(*key, CONFIG_GROUP_INFER_BATCH_SIZE))
        {
            ...
            if (init_params->maxBatchSize <= 0 || init_params->maxBatchSize > NVDSINFER_MAX_BATCH_SIZE)
            {
   ...
   * per-class detection parameters. */
     ...
     * all classes. */
        ...
        nvinfer->perClassDetectionFilterParams =
            new std::vector<GstNvInferDetectionFilterParams>(init_params->numDetectedClasses, detection_filter_params);
         ...
         * within the valid range of class_ids [0, numDetectedClasses - 1]. */
                ...
                {
                    g_printerr("Invalid group [%s]. Class attributes should be specified using group name '" CONFIG_GROUP_INFER_CLASS_ATTRS_PREFIX "<class-id>'\n", *group);
                    goto done;
                }
                ...
                {
                    g_printerr("Attributes specified for class %lu while element has been "

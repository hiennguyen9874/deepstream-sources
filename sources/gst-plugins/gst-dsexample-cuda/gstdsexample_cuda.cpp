...
static gboolean gst_dsexample_start(GstBaseTransform *btrans)
{
    ...
    CHECK_CUDA_STATUS(cudaMalloc(&dsexample->device_rgb_buf, dsexample->processing_width *
    ...
    dsexample->cvgpumat = new cv::cuda::GpuMat(
        dsexample->processing_height, dsexample->processing_width, CV_8UC3,
        (void *)dsexample->device_rgb_buf, dsexample->processing_width * RGB_BYTES_PER_PIXEL);
                                       ...
                                       gint input_height)
{
    ...
    */
    in_mat = cv::cuda::GpuMat(dsexample->processing_height, dsexample->processing_width, CV_8UC4,
                                  ...
                                  cv::cuda::GpuMat &in_mat)
{
    ...
    cv::Ptr<cv::cuda::Filter> gaussian_filter =
        cv::cuda::createGaussianFilter(CV_8UC4, CV_8UC4, cv::Size(15, 15), 0);
    ...
    if (dsexample->process_full_frame) {
...
#ifdef WITH_OPENCV
            output = DsExampleProcess(dsexample->dsexamplelib_ctx, dsexample->cvgpumat->data);
#else
            output =
        ...
        if (dsexample->blur_objects) {
            if (!(surface->memType == NVBUF_MEM_CUDA_DEVICE)) {
                GST_ELEMENT_ERROR(
                    dsexample, STREAM, FAILED,
                    ("%s:need NVBUF_MEM_CUDA_DEVICE memory for opencv cuda blurring", __func__),
                    (NULL));
                return GST_FLOW_ERROR;
            }
        }

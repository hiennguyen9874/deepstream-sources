...
static void gst_ds_nvmultiurisrc_bin_class_init(GstDsNvMultiUriBinClass *klass)
{
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_SENSOR_NAME_LIST,
        g_param_spec_string(
            "sensor-name-list", "optional comma separated list of source sensor names",
            "this vector is one to one mapped with the uri-list", NULL,
            (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | GST_PARAM_MUTABLE_READY)));
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_DISABLE_AUDIO,
        g_param_spec_boolean("disable-audio", "disable-audio", "Disable audio path at init time",
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_SEI_UUID,
        g_param_spec_string(
            "sei-uuid", "Set sei uuid", "Set sei uuid on decoder.", DEFAULT_SEI_UUID,
            (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | GST_PARAM_MUTABLE_READY)));
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_DROP_ON_LATENCY,
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_EXTRACT_SEI_TYPE5_DATA_DEC,
        g_param_spec_boolean(
            "extract-sei-type5-data-dec", "Extract SEI data for decoder",
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_LOW_LATENCY_MODE,
        g_param_spec_boolean(
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_BUFFER_MODE,
        g_param_spec_enum(
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_LEAKY,
        g_param_spec_enum(
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_MAX_SIZE_BUFFERS,
        g_param_spec_uint(
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_RTSP_INIT_RECONNECT_INTERVAL,
        g_param_spec_uint(
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_RTSP_RECONNECT_ATTEMPTS,
        g_param_spec_int(
            "rtsp-reconnect-attempts", "Set RTSP Reconnect attempts",
            "Set RTSP reconnect attempts ", G_MININT, G_MAXINT, DEFAULT_RTSP_RECONNECT_ATTEMPTS,
            (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | GST_PARAM_MUTABLE_READY)));
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_IPC_BUFFER_TIMESTAMP_COPY,
        g_param_spec_boolean(
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_IPC_SOCKET_PATH,
        g_param_spec_string(
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_IPC_CONNECTION_ATTEMPTS,
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_IPC_CONNECTION_INTERVAL,
    ...
    g_object_class_install_property(
        gobject_class, MULTIURIBIN_PROP_SENSORID_PADID_MAPPING,
        g_param_spec_boolean("sensorID-padID-mapping", "SensorID PadID Mapping",
                             "One to One Mapping between Sensor ID and Mux Pad ID, must start from "
                             "0 Eg: Camera0, Camera1, Camera2",
                             DEFAULT_SENSORID_PADID_MAPPING,
                             (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));
    ...
    g_object_class_install_property(
        gobject_class, PROP_EXTRACT_SEI_TYPE5_DATA_MUX,
        g_param_spec_boolean(
            "extract-sei-type5-data", "Extract SEI data for Muxer",
            "Set to extract and attach SEI type5 unregistered data on output buffer for Mux",
            DEFAULT_SEI_EXTRACT_DATA, (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));

    g_object_class_install_property(
        gobject_class, PROP_SORT_BATCH_BUFFERS,
        g_param_spec_boolean("sort-batch", "Sort Batch Buffer in ascending order of pad-ids",
                             "Sort Batch Buffer in ascending order", DEFAULT_SORT_BATCH_BUFFERS,
                             (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));

    g_object_class_install_property(
        gobject_class, PROP_CACHE_BUFFERS,
        g_param_spec_boolean("cache-buffer", "Render previous frame in case new frame is delayed",
                             ...
                             DEFAULT_CACHE_BATCH_BUFFERS,
                             (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));

    g_object_class_install_property(
        gobject_class, PROP_CACHE_BUFFERS_TIMEOUT,
        g_param_spec_int(
            "cache-buffer-timeout", "Cache Batch Buffer Timeout",
    ...
    g_object_class_install_property(
        gobject_class, PROP_EXTRACT_SIM_TIME,
        g_param_spec_boolean("extract-sei-sim-time", "extract-sei-sim-time",
                             "Set to extract and attach simulation time on output buffer",
                             DEFAULT_SEI_EXTRACT_SIM_TIME,
                             (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));

    g_object_class_install_property(
        gobject_class, PROP_ALIGN_FIRST_BUFFER,
        g_param_spec_boolean("align-first-buffer", "Align First Buffer",
                             ...
                             DEFAULT_ALIGN_FIRST_BATCH,
                             (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));

    g_object_class_install_property(
        gobject_class, PROP_SYNC_INPUTS_NTP,
        g_param_spec_int("sync-inputs-ntp", "PTS difference between incoming NTP frames",
                         "Time in microseconds to synchronize incoming inputs based on ntp", 0,
                         G_MAXINT, DEFAULT_SYNC_INPUTS_NTP,
                         (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));
                                                  ...
                                                  GParamSpec *pspec)
{
    ...
    case MULTIURIBIN_PROP_SENSOR_NAME_LIST:
        ...
        nvmultiurisrcbin->sensorNameListV = g_strsplit(nvmultiurisrcbin->sensorNameList, ",", -1);
        break;
...
static void gst_ds_nvmultiurisrc_bin_init(GstDsNvMultiUriBin *nvmultiurisrcbin)
{
    ...
    nvmultiurisrcbin->config->rtsp_reconnect_attempts = DEFAULT_RTSP_RECONNECT_ATTEMPTS;
                                                                  ...
                                                                  GstStateChange transition)
{
    ...
    if (transition == GST_STATE_CHANGE_NULL_TO_READY) {
        ...
        if (g_strlcat(binNameForCreator, (const gchar *)"_creator", binNameForCreatorLen + 10) <
        ...
        if (nvmultiurisrcbin->uriListV) {
            ...
            guint sensorIdListVLen = nvmultiurisrcbin->sensorIdListV
                                         ...
                                         : 0;
            guint sensorNameListVLen = nvmultiurisrcbin->sensorNameListV
                                           ? g_strv_length(nvmultiurisrcbin->sensorNameListV)
                                           : 0;
            ...
            for (i = 0; nvmultiurisrcbin->uriListV[i] != NULL; i++) {
                ...
                if (i < sensorNameListVLen && nvmultiurisrcbin->sensorNameListV[i]) {
                    nvmultiurisrcbin->config->sensorName = nvmultiurisrcbin->sensorNameListV[i];
                }
...
gpointer GThreadFuncRemoveSource(gpointer data)
{
     ...
     * Note: Multiple calls to gst_nvmultiurisrcbincreator_remove_source() is handled now in the
     * API.
     */
...
static void gst_ds_nvmultiurisrc_bin_handle_message(GstBin *bin, GstMessage *message)
{
    ...
    if (GST_MESSAGE_TYPE(message) == GST_MESSAGE_WARNING) {
        ...
        // "nvurisrc_bin_src_elem" is the uridecodebin of the nvurisrcbin. "src" is the rtspsrc of
        // the nvurisrcbin. In either case, the nvurisrcbin will be the src->parent
        if (g_strcmp0(GST_OBJECT_NAME(message->src), "nvurisrc_bin_src_elem") == 0 ||
            g_strcmp0(GST_OBJECT_NAME(message->src), "src") == 0) {
            nvurisrcbin = (GstDsNvUriSrcBin *)message->src->parent;
        }
        if (nvurisrcbin) {
            // In case of error from source set the rtsp reconnect interval value to
            // init-reconnect-interval-sec value (available via config)
                ...
                if (rtsp_info.attempt_exceeded) {
                    ...
                    if (ret == FALSE) {
                        GST_WARNING_OBJECT(ubin,
                                           "Failed to remove sensor; the sensor might have gotten "
                                           "removed already\n");
                    } else {
                        GST_DEBUG_OBJECT(ubin, "Successfully removed sensor\n");
                    }
...
static void s_stream_api_impl(NvDsServerStreamInfo *stream_info, void *ctx)
{
    ...
    if (stream_info->uri.find("/api/v1/") != std::string::npos) {
        ...
        if (g_strrstr(stream_info->value_change.c_str(), "add") ||
            ...
            nvmultiurisrcbin->config->sensorName = (gchar *)stream_info->value_camera_name.c_str();
            nvmultiurisrcbin->config->source_id = 0;
...
static void s_dec_api_impl(NvDsServerDecInfo *dec_info, void *ctx)
{
    GstDsNvMultiUriBin *nvmultiurisrcbin = (GstDsNvMultiUriBin *)ctx;
    (void)nvmultiurisrcbin;
...
static void s_infer_api_impl(NvDsServerInferInfo *infer_info, void *ctx)
{
    GstDsNvMultiUriBin *nvmultiurisrcbin = (GstDsNvMultiUriBin *)ctx;
    (void)nvmultiurisrcbin;
...
static void s_inferserver_api_impl(NvDsServerInferServerInfo *inferserver_info, void *ctx)
{
    GstDsNvMultiUriBin *nvmultiurisrcbin = (GstDsNvMultiUriBin *)ctx;
    (void)nvmultiurisrcbin;
...
static void s_nvtracker_api_impl(NvDsServerNvTrackerInfo *nvtracker_info, void *ctx)
{
    GstDsNvMultiUriBin *nvmultiurisrcbin = (GstDsNvMultiUriBin *)ctx;
    (void)nvmultiurisrcbin;
        ...
        if (!find_source(nvmultiurisrcbin->nvmultiurisrcbinCreator, sourceId)) {
            if (nvtracker_info->nvTracker_flag == NVTRACKER_CONFIG) {
                ...
                nvtracker_info->nvTracker_log =
                    "NVTRACKER_CONFIG_UPDATE_FAIL, Unable to find stream id for nvTracker property "
                    "updation";
            }
        } else {
            GstEvent *nvevent =
                gst_nvevent_nvtracker_config_update((char *)nvtracker_info->stream_id.c_str(),
            ...
            if (!gst_pad_push_event((GstPad *)(nvmultiurisrcbin->bin_src_pad), nvevent)) {
                ...
                nvtracker_info->status = NVTRACKER_CONFIG_UPDATE_FAIL;
                nvtracker_info->nvTracker_log =
                    "NVTRACKER_CONFIG_UPDATE_FAIL, nv-tracker-config-update event not pushed";
                nvtracker_info->err_info.code = StatusInternalServerError;
            } else {
...
static void s_conv_api_impl(NvDsServerConvInfo *conv_info, void *ctx)
{
    GstDsNvMultiUriBin *nvmultiurisrcbin = (GstDsNvMultiUriBin *)ctx;
    (void)nvmultiurisrcbin;
    ...
    if (conv_info->uri.find("/api/v1/") != std::string::npos) {
        if (!set_nvuribin_conv_prop(nvmultiurisrcbin->nvmultiurisrcbinCreator, sourceId,
                                    conv_info)) {
...
static void s_mux_api_impl(NvDsServerMuxInfo *mux_info, void *ctx)
{
    GstDsNvMultiUriBin *nvmultiurisrcbin = (GstDsNvMultiUriBin *)ctx;
    (void)nvmultiurisrcbin;
...
static void s_enc_api_impl(NvDsServerEncInfo *enc_info, void *ctx)
{
    GstDsNvMultiUriBin *nvmultiurisrcbin = (GstDsNvMultiUriBin *)ctx;
    (void)nvmultiurisrcbin;
...
static void s_osd_api_impl(NvDsServerOsdInfo *osd_info, void *ctx)
{
    GstDsNvMultiUriBin *nvmultiurisrcbin = (GstDsNvMultiUriBin *)ctx;
    (void)nvmultiurisrcbin;
...
static void s_analytics_api_impl(NvDsServerAnalyticsInfo *analytics_info, void *ctx)
{
    GstDsNvMultiUriBin *nvmultiurisrcbin = (GstDsNvMultiUriBin *)ctx;

    if (analytics_info->uri.find("/api/v1/") != std::string::npos) {
        GstEvent *nvevent = gst_nvevent_analytics_reload_config_update(
            (char *)analytics_info->config_file_path.c_str());

        if (!nvevent) {
            analytics_info->status = RELOAD_CONFIG_UPDATE_FAIL;
            analytics_info->analytics_log =
                "RELOAD_CONFIG_UPDATE_FAIL, analytics-config-update event creation failed";
        ...
        if (!gst_pad_push_event((GstPad *)(nvmultiurisrcbin->bin_src_pad), nvevent)) {
            analytics_info->status = RELOAD_CONFIG_UPDATE_FAIL;
            analytics_info->analytics_log =
                "RELOAD_CONFIG_UPDATE_FAIL, analytics-config-update event not pushed";
            analytics_info->err_info.code = StatusInternalServerError;
        } else {
...
static void s_appinstance_api_impl(NvDsServerAppInstanceInfo *appinstance_info, void *ctx)
{
    GstDsNvMultiUriBin *nvmultiurisrcbin = (GstDsNvMultiUriBin *)ctx;
    (void)nvmultiurisrcbin;
...
static void s_get_request_api_impl(NvDsServerGetRequestInfo *get_request_info, void *ctx)
{
    GstDsNvMultiUriBin *nvmultiurisrcbin = (GstDsNvMultiUriBin *)ctx;
    (void)nvmultiurisrcbin;
    ...
    if (get_request_info->uri.find("/api/v1/") != std::string::npos) {
        ...
        if (gst_nvmultiurisrcbincreator_get_source_info_list(
            ...
            if (get_request_info->get_request_flag == GET_LIVE_STREAM_INFO) {
                ...
                gst_element_get_state(GST_ELEMENT(nvmultiurisrcbin), &curr_state, &pending_state,
                                      GST_CLOCK_TIME_NONE);
...
static void rest_api_server_start(GstDsNvMultiUriBin *nvmultiurisrcbin)
{
    ...
    server_cb.stream_cb = [nvmultiurisrcbin](NvDsServerStreamInfo *stream_info, void *ctx) {
    ...
    server_cb.inferserver_cb = [nvmultiurisrcbin](NvDsServerInferServerInfo *inferserver_info,
                                                  void *ctx) {
        s_inferserver_api_impl(inferserver_info, (void *)nvmultiurisrcbin);
    };
    server_cb.nvTracker_cb = [nvmultiurisrcbin](NvDsServerNvTrackerInfo *nvtracker_info,
                                                void *ctx) {
    ...
    server_cb.analytics_cb = [nvmultiurisrcbin](NvDsServerAnalyticsInfo *analytics_info,
                                                void *ctx) {
        s_analytics_api_impl(analytics_info, (void *)nvmultiurisrcbin);
    };

    server_cb.appinstance_cb = [nvmultiurisrcbin](NvDsServerAppInstanceInfo *appinstance_info,
                                                  void *ctx) {
        s_appinstance_api_impl(appinstance_info, (void *)nvmultiurisrcbin);
    };

    server_cb.get_request_cb = [nvmultiurisrcbin](NvDsServerGetRequestInfo *get_request_info,
                                                  void *ctx) {
...
gboolean gGstNvMultiUriSrcBinStaticInit()
{
    return gst_plugin_register_static(GST_VERSION_MAJOR, GST_VERSION_MINOR,
                                      "nvdsgst_deepstream_bins2", DESCRIPTION, plugin_init_2, "8.0",

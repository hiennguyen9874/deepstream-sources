...
struct cuOSDContextImpl : public cuOSDContext {
    ...
    unique_ptr<Memory<int>> gpu_commands_offset; // sizeof(TextCommand), sizeof(TextCommand) +
...
static inline tuple<unsigned char, unsigned char, unsigned char> make_u83(unsigned char a,
                                                                          unsigned char b,
                                                                          unsigned char c)
{
...
void cuosd_measure_text(cuOSDContext_t _context,
...
void cuosd_draw_text(cuOSDContext_t _context,
                     const char *utf8_text,
                     int font_size,
                     const char *font,
                     int x,
                     ...
                     cuOSDColor bg_color)
{
    ...
    auto precompute_line = [&](const std::vector<unsigned long> &line_words,
        ...
        commands.emplace_back(make_shared<TextHostCommand>(
            line_words, font_size, font, x + xmargin, ypos + ymargin - yoffset, border_color.r,
    ...
    if (!need_split_lines) {
        ...
        if (need_background_fill) {
            cuosd_draw_rectangle(_context, background_min_x, background_min_y, background_max_x,
                                 background_max_y, -1, bg_color);
        }
...
void cuosd_draw_clock(cuOSDContext_t _context,
                      cuOSDClockFormat clock_format,
                      long time,
                      int font_size,
                      const char *font,
                      int x,
...
void cuosd_draw_rectangle(cuOSDContext_t _context,
                          int left,
                          int top,
                          int right,
                          int bottom,
...
void cuosd_draw_boxblur(cuOSDContext_t _context,
                        int left,
                        int top,
                        int right,
                        int bottom,
                        int kernel_size)
{
    ...
    if (right - left < 3 || bottom - top < 3 || kernel_size < 1) {
        CUOSD_PRINT_E(
            "[cuosd_draw_boxblur] This operation will be ignored because the region of interest is "
            "too small, or the kernel is too small.");
        return;
    }
...
void cuosd_draw_polyline(cuOSDContext_t _context,
                         int *h_pts,
                         int *d_pts,
                         int n_pts,
                         int thickness,
                         ...
                         cuOSDColor fill_color)
{
    ...
    for (int i = 1; i < n_pts; i++) {
        ...
        cuosd_draw_line(context, h_pts[2 * i - 2], h_pts[2 * i - 1], x, y, thickness, border_color,
                        interpolation);
        nline++;
    }
    ...
    if (n_pts > 2) {
        if (is_closed) {
            cuosd_draw_line(context, h_pts[0], h_pts[1], h_pts[2 * n_pts - 2], h_pts[2 * n_pts - 1],
...
void cuosd_draw_segmentmask(cuOSDContext_t _context,
                            int left,
                            int top,
                            int right,
                            int bottom,
...
void cuosd_draw_line(cuOSDContext_t _context,
                     int x0,
                     int y0,
                     int x1,
                     int y1,
...
void cuosd_draw_point(cuOSDContext_t _context, int cx, int cy, int radius, cuOSDColor color)
{
    cuosd_draw_circle(_context, cx, cy, radius, -1, color);
}

void cuosd_draw_circle(cuOSDContext_t _context,
                       int cx,
                       int cy,
                       int radius,
                       int thickness,
                       ...
                       cuOSDColor bg_color)
{
    ...
    context->commands.emplace_back(make_shared<CircleCommand>(
        cx, cy, radius, thickness, border_color.r, border_color.g, border_color.b, border_color.a));
}
...
void cuosd_draw_nv12_source(cuOSDContext_t _context,
                            int left,
                            int top,
                            int right,
                            int bottom,
...
static void cuosd_text_perper(cuOSDContext_t _context, int width, int height, void *_stream)
{
    cuOSDContextImpl *context = (cuOSDContextImpl *)_context;
    ...
    for (auto &cmd : context->commands) {
        if (cmd->type == CommandType::Text) {
            ...
            auto glyph_map =
                context->text_backend->query(text_cmd->font_name.c_str(), text_cmd->font_size);
            ...
            for (auto &word : text_cmd->text) {
                ...
                location.image_y =
                    text_cmd->y + context->text_backend->compute_y_offset(max_glyph_height, h, meta,
                                                                          text_cmd->font_size);
...
void cuosd_apply(cuOSDContext_t _context,
                 void *data0,
                 void *data1,
                 int width,
                 int stride,
...
void cuosd_launch(cuOSDContext_t _context,
                  void *data0,
                  void *data1,
                  int width,
                  int stride,
                  ...
                  void *stream)
{
    ...
    cuosd_launch_kernel(
        ...
        context->gpu_blur_commands ? (const unsigned char *)context->gpu_blur_commands->device()
                                   : nullptr,

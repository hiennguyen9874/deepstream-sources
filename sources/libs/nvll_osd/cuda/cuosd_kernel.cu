...
static __device__ void __forceinline__
yuv2rgb(int y, int u, int v, uint8_t &r, uint8_t &g, uint8_t &b)
{
...
static __device__ void __forceinline__
rgb2yuv(int r, int g, int b, uint8_t &y, uint8_t &u, uint8_t &v)
{
...
//      |     |
//      b --- c
static __device__ __forceinline__ bool inbox_single_pixel(float ix,
                                                          float iy,
                                                          float ax,
                                                          float ay,
                                                          float bx,
...
static __device__ void blend_single_color(uchar4 &color,
                                          unsigned char &c0,
                                          unsigned char &c1,
                                          unsigned char &c2,
                                          unsigned char a)
{
    ...
    color.x = u8cast(
        (((color.x * background_alpha * (255 - foreground_alpha)) >> 8) + (c0 * foreground_alpha)) /
        blend_alpha);
    color.y = u8cast(
        (((color.y * background_alpha * (255 - foreground_alpha)) >> 8) + (c1 * foreground_alpha)) /
        blend_alpha);
    color.z = u8cast(
        (((color.z * background_alpha * (255 - foreground_alpha)) >> 8) + (c2 * foreground_alpha)) /
        blend_alpha);
    color.w = blend_alpha;
}
...
TextCommand::TextCommand(int text_line_size,
                         int ilocation,
                         unsigned char c0,
                         unsigned char c1,
                         unsigned char c2,
...
static __device__ unsigned char interpolation_fn(float x,
                                                 int a,
                                                 int b,
                                                 int padding,
...
static __device__ __forceinline__ bool external_msaa4x(float ix,
                                                       float iy,
                                                       float ax,
                                                       float ay,
                                                       float bx,
...
static __device__ __forceinline__ bool internal_msaa4x(float ix,
                                                       float iy,
                                                       float ax,
                                                       float ay,
                                                       float bx,
...
static __device__ void render_rectangle_fill_msaa4x(int ix,
                                                    int iy,
                                                    RectangleCommand *p,
                                                    uchar4 color[4])
{
    unsigned char alpha;
    if (internal_msaa4x(ix, iy, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1, p->dy1,
                        p->c3, alpha)) {
        blend_single_color(color[0], p->c0, p->c1, p->c2, alpha);
    }
    if (internal_msaa4x(ix + 1, iy, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1, p->dy1,
                        p->c3, alpha)) {
        blend_single_color(color[1], p->c0, p->c1, p->c2, alpha);
    }
    if (internal_msaa4x(ix, iy + 1, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1, p->dy1,
                        p->c3, alpha)) {
        blend_single_color(color[2], p->c0, p->c1, p->c2, alpha);
    }
    if (internal_msaa4x(ix + 1, iy + 1, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1,
                        p->dy1, p->c3, alpha)) {
...
static __device__ void render_rectangle_fill(int ix, int iy, RectangleCommand *p, uchar4 color[4])
{
    ...
    if (inbox_single_pixel(ix + 1, iy, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1,
                           p->dy1)) {
        blend_single_color(color[1], p->c0, p->c1, p->c2, p->c3);
    }
    if (inbox_single_pixel(ix, iy + 1, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1,
                           p->dy1)) {
        blend_single_color(color[2], p->c0, p->c1, p->c2, p->c3);
    }
    if (inbox_single_pixel(ix + 1, iy + 1, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1,
                           p->dy1)) {
...
static __device__ void render_rectangle_border_msaa4x(int ix,
                                                      int iy,
                                                      RectangleCommand *p,
                                                      uchar4 color[4])
{
    ...
    if (internal_msaa4x(ix, iy, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1, p->dy1,
                        p->c3, alpha) &&
        external_msaa4x(ix, iy, p->ax2, p->ay2, p->bx2, p->by2, p->cx2, p->cy2, p->dx2, p->dy2,
                        p->c3, alpha)) {
        blend_single_color(color[0], p->c0, p->c1, p->c2, alpha);
    }
    if (internal_msaa4x(ix + 1, iy, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1, p->dy1,
                        p->c3, alpha) &&
        external_msaa4x(ix + 1, iy, p->ax2, p->ay2, p->bx2, p->by2, p->cx2, p->cy2, p->dx2, p->dy2,
                        p->c3, alpha)) {
        blend_single_color(color[1], p->c0, p->c1, p->c2, alpha);
    }
    if (internal_msaa4x(ix, iy + 1, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1, p->dy1,
                        p->c3, alpha) &&
        external_msaa4x(ix, iy + 1, p->ax2, p->ay2, p->bx2, p->by2, p->cx2, p->cy2, p->dx2, p->dy2,
                        p->c3, alpha)) {
        blend_single_color(color[2], p->c0, p->c1, p->c2, alpha);
    }
    if (internal_msaa4x(ix + 1, iy + 1, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1,
                        p->dy1, p->c3, alpha) &&
        external_msaa4x(ix + 1, iy + 1, p->ax2, p->ay2, p->bx2, p->by2, p->cx2, p->cy2, p->dx2,
                        p->dy2, p->c3, alpha)) {
        blend_single_color(color[3], p->c0, p->c1, p->c2, alpha);
    }
}
...
{
    if (!inbox_single_pixel(ix, iy, p->ax2, p->ay2, p->bx2, p->by2, p->cx2, p->cy2, p->dx2,
                            p->dy2) &&
        inbox_single_pixel(ix, iy, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1,
                           p->dy1)) {
        blend_single_color(color[0], p->c0, p->c1, p->c2, p->c3);
    }
    if (!inbox_single_pixel(ix + 1, iy, p->ax2, p->ay2, p->bx2, p->by2, p->cx2, p->cy2, p->dx2,
                            p->dy2) &&
        inbox_single_pixel(ix + 1, iy, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1,
                           p->dy1)) {
        blend_single_color(color[1], p->c0, p->c1, p->c2, p->c3);
    }
    if (!inbox_single_pixel(ix, iy + 1, p->ax2, p->ay2, p->bx2, p->by2, p->cx2, p->cy2, p->dx2,
                            p->dy2) &&
        inbox_single_pixel(ix, iy + 1, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1,
                           p->dy1)) {
        blend_single_color(color[2], p->c0, p->c1, p->c2, p->c3);
    }
    if (!inbox_single_pixel(ix + 1, iy + 1, p->ax2, p->ay2, p->bx2, p->by2, p->cx2, p->cy2, p->dx2,
                            p->dy2) &&
        inbox_single_pixel(ix + 1, iy + 1, p->ax1, p->ay1, p->bx1, p->by1, p->cx1, p->cy1, p->dx1,
                           p->dy1)) {
        blend_single_color(color[3], p->c0, p->c1, p->c2, p->c3);
    }
}
...
static __device__ void render_circle_interpolation(int ix,
                                                   int iy,
                                                   CircleCommand *p,
                                                   uchar4 color[4])
{
    ...
    float tr3 =
        sqrt((float)(ix + 1 - p->cx) * (ix + 1 - p->cx) + (iy + 1 - p->cy) * (iy + 1 - p->cy));
...
static __device__ void sample_pixel_bilinear(float *d_ptr,
                                             int x,
                                             int y,
                                             float sx,
                                             float sy,
...
{
    if (ix + 1 < p->bounding_left || iy + 1 < p->bounding_top || ix >= p->bounding_right ||
        iy >= p->bounding_bottom)
        return;

    int sinsc[4] = {0, 0, 0, 0};
    for (int i = 0; i < p->n_pts; i++) {
        if (i == 0) {
            if (isRayIntersectsSegment(ix, iy, p->d_pts[0], p->d_pts[1], p->d_pts[p->n_pts * 2 - 2],
                                       p->d_pts[p->n_pts * 2 - 1]))
                sinsc[0] += 1;
            if (isRayIntersectsSegment(ix + 1, iy, p->d_pts[0], p->d_pts[1],
                                       p->d_pts[p->n_pts * 2 - 2], p->d_pts[p->n_pts * 2 - 1]))
                sinsc[1] += 1;
            ...
            if (isRayIntersectsSegment(ix, iy, p->d_pts[i * 2 - 2], p->d_pts[i * 2 - 1],
                ...
                sinsc[0] += 1;
            if (isRayIntersectsSegment(ix + 1, iy, p->d_pts[i * 2 - 2], p->d_pts[i * 2 - 1],
                                       p->d_pts[i * 2], p->d_pts[i * 2 + 1]))
                sinsc[1] += 1;
            if (isRayIntersectsSegment(ix, iy + 1, p->d_pts[i * 2 - 2], p->d_pts[i * 2 - 1],
                                       p->d_pts[i * 2], p->d_pts[i * 2 + 1]))
                sinsc[2] += 1;
            if (isRayIntersectsSegment(ix + 1, iy + 1, p->d_pts[i * 2 - 2], p->d_pts[i * 2 - 1],
                                       p->d_pts[i * 2], p->d_pts[i * 2 + 1]))
                sinsc[3] += 1;
        }
    }
...
static __device__ void render_segment_bilinear(int ix, int iy, SegmentCommand *p, uchar4 color[4])
{
    ...
    unsigned char alpha3 = ix + 1 < p->bounding_left || iy + 1 < p->bounding_top ||
                                   ix + 1 >= p->bounding_right || iy + 1 >= p->bounding_bottom
                               ? 0
                               : 127;
    ...
    if (alpha0) {
        sample_pixel_bilinear(p->d_seg, fx, fy, p->scale_x, p->scale_y, p->seg_width, p->seg_height,
                              p->seg_threshold, alpha0);
        blend_single_color(color[0], p->c0, p->c1, p->c2, alpha0);
    }
    ...
    if (alpha3) {
        sample_pixel_bilinear(p->d_seg, fx + 1, fy + 1, p->scale_x, p->scale_y, p->seg_width,
                              p->seg_height, p->seg_threshold, alpha3);
        blend_single_color(color[3], p->c0, p->c1, p->c2, alpha3);
    }
}

static __device__ bool render_text(int ix,
                                   int iy,
                                   ...
                                   unsigned char &a)
{
    if (ix + 1 < location.image_x || iy + 1 < location.image_y ||
        ix >= location.image_x + location.text_w || iy >= location.image_y + location.text_h)
        return false;
...
static __device__ void blend_nv12_bilinear(void *d_ptr0,
                                           void *d_ptr1,
                                           int x,
                                           int y,
                                           float sx,
                                           ...
                                           bool block_linear)
{
    ...
    if (block_linear) {
        ...
        _scr[0].y =
            surf2Dread<unsigned char>((cudaSurfaceObject_t)d_ptr1, 2 * (x_low >> 1), y_low >> 1);
        _scr[1].y =
            surf2Dread<unsigned char>((cudaSurfaceObject_t)d_ptr1, 2 * (x_high >> 1), y_low >> 1);
        _scr[2].y =
            surf2Dread<unsigned char>((cudaSurfaceObject_t)d_ptr1, 2 * (x_low >> 1), y_high >> 1);
        _scr[3].y =
            surf2Dread<unsigned char>((cudaSurfaceObject_t)d_ptr1, 2 * (x_high >> 1), y_high >> 1);

        _scr[0].z = surf2Dread<unsigned char>((cudaSurfaceObject_t)d_ptr1, 2 * (x_low >> 1) + 1,
                                              y_low >> 1);
        _scr[1].z = surf2Dread<unsigned char>((cudaSurfaceObject_t)d_ptr1, 2 * (x_high >> 1) + 1,
                                              y_low >> 1);
        _scr[2].z = surf2Dread<unsigned char>((cudaSurfaceObject_t)d_ptr1, 2 * (x_low >> 1) + 1,
                                              y_high >> 1);
        _scr[3].z = surf2Dread<unsigned char>((cudaSurfaceObject_t)d_ptr1, 2 * (x_high >> 1) + 1,
                                              y_high >> 1);
    } else {
        _scr[0] = make_uchar4(
        ...
        _scr[3] = make_uchar4(
            ...
            ((unsigned char *)d_ptr1)[(y_high >> 1) * stride + 2 * (x_high >> 1) + 1], c3);
    }
...
static __device__ void render_nv12_src(int ix, int iy, NV12SourceCommand *p, uchar4 color[4])
{
    if (ix + 1 < p->bounding_left || iy + 1 < p->bounding_top || ix >= p->bounding_right ||
        iy >= p->bounding_bottom)
        return;
    ...
    if (alpha0) {
        blend_nv12_bilinear(p->d_src0, p->d_src1, fx, fy, p->scale_x, p->scale_y, p->src_width,
                            p->src_stride, p->src_height, &color[0], p->c3, p->block_linear);
    }

    if (alpha1) {
        blend_nv12_bilinear(p->d_src0, p->d_src1, fx + 1, fy, p->scale_x, p->scale_y, p->src_width,
                            p->src_stride, p->src_height, &color[1], p->c3, p->block_linear);
    }

    if (alpha2) {
        blend_nv12_bilinear(p->d_src0, p->d_src1, fx, fy + 1, p->scale_x, p->scale_y, p->src_width,
                            p->src_stride, p->src_height, &color[2], p->c3, p->block_linear);
    }
...
static __device__ void blend_rgba_bilinear(uint8_t *d_ptr,
                                           int x,
                                           int y,
                                           float sx,
                                           float sy,
...
static __device__ void render_rgba_src(int ix, int iy, RGBASourceCommand *p, uchar4 color[4])
{
    if (ix + 1 < p->bounding_left || iy + 1 < p->bounding_top || ix >= p->bounding_right ||
        iy >= p->bounding_bottom)
        return;
    ...
    if (alpha0) {
        blend_rgba_bilinear((uint8_t *)p->d_src, fx, fy, p->scale_x, p->scale_y, p->src_width,
                            p->src_stride, p->src_height, &color[0]);
    }

    if (alpha1) {
        blend_rgba_bilinear((uint8_t *)p->d_src, fx + 1, fy, p->scale_x, p->scale_y, p->src_width,
                            p->src_stride, p->src_height, &color[1]);
    }

    if (alpha2) {
        blend_rgba_bilinear((uint8_t *)p->d_src, fx, fy + 1, p->scale_x, p->scale_y, p->src_width,
                            p->src_stride, p->src_height, &color[2]);
    }
...
struct BlendingPixel<ImageFormat::RGBA> {
    static __device__ void call(const void *image0,
                                const void *image1,
                                int x,
                                int y,
                                int stride,
                                uchar4 plot_colors[4])
    {
        for (int i = 0; i < 2; ++i) {
            ...
            for (int j = 0; j < 2; ++j, p += 4) {
                ...
                p[0] = u8cast((((p[0] * background_alpha * (255 - foreground_alpha)) >> 8) +
                               (rcolor.x * foreground_alpha)) /
                              blend_alpha);
                p[1] = u8cast((((p[1] * background_alpha * (255 - foreground_alpha)) >> 8) +
                               (rcolor.y * foreground_alpha)) /
                              blend_alpha);
                p[2] = u8cast((((p[2] * background_alpha * (255 - foreground_alpha)) >> 8) +
...
struct BlendingPixel<ImageFormat::BlockLinearNV12> {
                                ...
                                uchar4 plot_colors[4])
    {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            int blend_alpha =
                ((background_alpha * (255 - foreground_alpha)) >> 8) + foreground_alpha;
            lcolor.x = u8cast((((lcolor.x * background_alpha * (255 - foreground_alpha)) >> 8) +
                               (rcolor.x * foreground_alpha)) /
                              blend_alpha);
            lcolor.y = u8cast((((lcolor.y * background_alpha * (255 - foreground_alpha)) >> 8) +
                               (rcolor.y * foreground_alpha)) /
                              blend_alpha);
            lcolor.z = u8cast((((lcolor.z * background_alpha * (255 - foreground_alpha)) >> 8) +
                               (rcolor.z * foreground_alpha)) /
                              blend_alpha);
        }
...
static __global__ void render_elements_kernel(int bx,
                                              int by,
                                              const TextLocation *text_locations,
                                              const unsigned char *text_bitmap,
                                              int text_bitmap_width,
                                              const int *line_location_base,
                                              const unsigned char *commands,
                                              const int *command_offsets,
                                              int num_command,
                                              const void *image0,
                                              const void *image1,
                                              int image_width,
    ...
    for (int i = 0; i < num_command; ++i) {
        ...
        case CommandType::Text: {
            ...
            for (int j = ilocation_begin; j < ilocation_end; ++j) {
                bool hit = render_text(ix, iy, text_locations[j], text_bitmap, text_bitmap_width,
                                       context_color, pcommand->c0, pcommand->c1, pcommand->c2,
                                       pcommand->c3);
                if (hit)
                    break;
            }
...
template <ImageFormat format>
static __device__ void __forceinline__ load_pixel(const void *luma,
                                                  const void *chroma,
                                                  int x,
                                                  int y,
                                                  int down_x,
...
template <>
__device__ void __forceinline__ load_pixel<ImageFormat::BlockLinearNV12>(const void *luma,
                                                                         const void *chroma,
                                                                         int x,
                                                                         int y,
                                                                         int down_x,
...
template <>
__device__ void __forceinline__ load_pixel<ImageFormat::PitchLinearNV12>(const void *luma,
                                                                         const void *chroma,
                                                                         int x,
                                                                         int y,
                                                                         int down_x,
...
template <>
__device__ void __forceinline__ load_pixel<ImageFormat::RGBA>(const void *luma,
                                                              const void *chroma,
                                                              int x,
                                                              int y,
                                                              int down_x,
...
template <ImageFormat format>
static __device__ void __forceinline__ save_pixel(void *luma,
                                                  void *chroma,
                                                  int x,
                                                  int y,
                                                  int down_x,
...
// BL sample pixel implmentation
template <>
__device__ void __forceinline__ save_pixel<ImageFormat::BlockLinearNV12>(void *luma,
                                                                         void *chroma,
                                                                         int x,
                                                                         int y,
                                                                         int down_x,
...
// PL sample pixel implmentation
template <>
__device__ void __forceinline__ save_pixel<ImageFormat::PitchLinearNV12>(void *luma,
                                                                         void *chroma,
                                                                         int x,
                                                                         int y,
                                                                         int down_x,
...
template <>
__device__ void __forceinline__ save_pixel<ImageFormat::RGB>(void *luma,
                                                             void *chroma,
                                                             int x,
                                                             int y,
                                                             int down_x,
...
template <>
__device__ void __forceinline__ save_pixel<ImageFormat::RGBA>(void *luma,
                                                              void *chroma,
                                                              int x,
                                                              int y,
                                                              int down_x,
...
template <ImageFormat format>
static __global__ void render_blur_kernel(const BoxBlurCommand *commands,
                                          int num_command,
                                          void *image0,
                                          void *image1,
                                          int image_width,
                                          ...
                                          int image_height)
{
    ...
    auto &pix = crop[iy][ix];
    load_pixel<format>(image0, image1, sx, sy, round_down2(sx), image_width, stride, pix.x, pix.y,
                       pix.z);
    __syncthreads();
        ...
        for (int j = 0; j < gap_width; ++j) {
            ...
            if (fx >= 0 && fx < image_width && fy >= 0 && fy < image_height) {
                ...
                if (sx < 32 && sy < 32) {
                    ...
                    save_pixel<format>(image0, image1, fx, fy, round_down2(fx), image_width, stride,
                                       pix.x, pix.y, pix.z);
                }
            }
        }
...
typedef void (*cuosd_launch_kernel_impl_fptr)(void *image_data0,
                                              void *image_data1,
                                              int width,
                                              int stride,
                                              int height,
...
typedef void (*cuosd_launch_blur_kernel_impl_fptr)(void *image_data0,
                                                   void *image_data1,
                                                   int width,
                                                   int stride,
                                                   int height,
...
template <ImageFormat format, bool have_rotate_msaa>
static void cuosd_launch_kernel_impl(void *image_data0,
                                     void *image_data1,
                                     int width,
                                     int stride,
                                     int height,
                                     ...
                                     void *_stream)
{
    ...
    if (bounding_width < 1 || bounding_height < 1) {
        CUOSD_PRINT_W(
            "Please check if there is anything to draw, or cuosd_apply has been called\n");
        return;
    }
    ...
    render_elements_kernel<format, have_rotate_msaa><<<grid, block, 0, stream>>>(
        ...
        line_location_base, commands, commands_offset, num_commands, image_data0, image_data1,
        width, stride, height);
...
template <ImageFormat format>
static void cuosd_launch_blur_kernel_impl(void *image_data0,
                                          void *image_data1,
                                          int width,
                                          int stride,
                                          int height,
                                          ...
                                          void *_stream)
{
    ...
    render_blur_kernel<format><<<grid, block, 0, stream>>>(commands, num_commands, image_data0,
                         ...
                         void *_stream)
{
    ...
    if (num_commands > 0) {
        ...
        func_list[index](image_data0, image_data1, width, stride, height, text_location,
                         ...
                         commands_offset, num_commands, bounding_left, bounding_top, bounding_right,
                         bounding_bottom, _stream);
    }
}

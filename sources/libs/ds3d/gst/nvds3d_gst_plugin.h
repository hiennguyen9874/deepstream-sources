...
DS3D_EXPORT_API ds3d::ErrCode NvDs3D_GstAppSrcSetDataloader(GstAppSrc *src,
...
template <class GuardProcess>
inline ErrCode loadCustomProcessor(const config::ComponentConfig &compConfig,
                                   GuardProcess &customProcessor,
                                   Ptr<CustomLibFactory> &lib)
{
    ...
    processor.reset(customLib->CreateCtx<abiRefType>(compConfig.customLibPath,
                                                     compConfig.customCreateFunction));
    DS3D_FAILED_RETURN(processor, ErrCode::kLoadLib,
                       "create custom processor: %s from lib: %s failed",
 ...
 * @return ds3d::ErrCode
 */
inline ErrCode NvDs3D_CreateDataLoaderSrc(const config::ComponentConfig &compConfig,
                                          DataLoaderSrc &loaderSrc,
                                          bool start)
{
    GuardDataLoader loader;
    DS3D_ERROR_RETURN(loadCustomProcessor(compConfig, loader, loaderSrc.customlib),
    ...
    DS_ASSERT(appSrc);
    DS3D_ERROR_RETURN(NvDs3D_GstAppSrcSetDataloader(appSrc, loader.release()),
                      "Set dataloader into GstAppsrc failed.");

    return ErrCode::kGood;
}

inline ErrCode NvDs3D_CreateDataRenderSink(const config::ComponentConfig &compConfig,
                                           DataRenderSink &renderSink,
                                           bool start)
{
    GuardDataRender render;
    DS3D_ERROR_RETURN(loadCustomProcessor(compConfig, render, renderSink.customlib),
    ...
    DS3D_ERROR_RETURN(config::CatchYamlCall(setGstProperties),
                      "parse gst_properties failed for datareander");
    ...
    DS_ASSERT(appSink);
    DS3D_ERROR_RETURN(NvDs3D_GstAppSinkSetDataRender(appSink, render.release()),

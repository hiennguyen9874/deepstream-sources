                         ...
                         void *stream)
{
    ...
    if (device) {
        checkRuntime(cudaMemcpyAsync(output.ptr(), data, output.bytes(), cudaMemcpyDeviceToDevice,
                                     (cudaStream_t)stream));
    } else {
        checkRuntime(cudaMemcpyAsync(output.ptr(), data, output.bytes(), cudaMemcpyHostToHost,
                                     (cudaStream_t)stream));
    }
    return output;
}
...
Tensor Tensor::loadbinary(const std::string &file,
                          ...
                          bool device)
{
    ...
    if (num_implicit_dim > 0) {
        ...
        if (fsize % explicit_bytes != 0) {
            printf(
                "Cannot be calculated the implicit dimension, and the file bytes[%ld] cannot be "
                "divided "
                "by the size of the "
    ...
    if (bytes != fsize) {
        printf(
            "Cannot be loaded by the specified shape, The file has %ld byte and the shape requires "
            "%ld "
            "byte\n",
            fsize, bytes);
        fclose(f);
...
Tensor Tensor::load(const std::string &file, bool device, void *stream)
{
    ...
    Tensor output = Tensor::create(shape, dtype, device);
    if (device) {
        checkRuntime(cudaMemcpyAsync(output.ptr(), host_data.data(), bytes, cudaMemcpyHostToDevice,
                                     (cudaStream_t)stream));
    } else {
        checkRuntime(cudaMemcpyAsync(output.ptr(), host_data.data(), bytes, cudaMemcpyHostToHost,
                                     (cudaStream_t)stream));
    }
...
{
    printf("%s[%s] %s%s", prefix, dtype_string(dtype()), format_shape(shape).c_str(),
           lines == 1 ? ": " : ": \n");
    ...
    if (this->device()) {
        tensor_data =
            shared_ptr<TensorData>(TensorData::create(this->bytes(), this->dtype(), false));
        checkRuntime(
            cudaMemcpy(tensor_data->data, this->ptr(), this->bytes(), cudaMemcpyDeviceToHost));
    }
    ...
    if (this->dtype() == DataType::Float32) {
        ...
        for (size_t i = 0; i < num_print; ++i) {
            printf("%.3f ",
                   _native_half2float(*((unsigned short *)tensor_data->data + offset + i)));
            if ((i + 1) % num_per_line == 0)
                printf("\n");
        }
...
void Tensor::copy_from_host(const void *data, void *stream)
{
    ...
    if (this->device()) {
        checkRuntime(
            cudaMemcpyAsync(this->ptr(), data, this->bytes(), cudaMemcpyHostToDevice, _stream));
    } else {
        checkRuntime(
            cudaMemcpyAsync(this->ptr(), data, this->bytes(), cudaMemcpyHostToHost, _stream));
    }
}
...
void Tensor::copy_from_device(const void *data, void *stream)
{
    ...
    if (this->device()) {
        ...
        checkRuntime(
            cudaMemcpyAsync(this->ptr(), data, this->bytes(), cudaMemcpyDeviceToHost, _stream));
    }
}
...
Tensor Tensor::to_half(void *_stream) const
{
    ...
    DISPATCH_BY_TYPES(this->dtype(), [&] {
        cuda_linear_launch(any_to_any_device, stream, this->numel, this->ptr<scalar_t>(),
                           output.ptr<half>());
        checkRuntime(cudaStreamSynchronize(stream));
    });
    return output;

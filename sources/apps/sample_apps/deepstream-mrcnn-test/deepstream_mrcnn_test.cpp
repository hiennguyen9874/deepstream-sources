...
static gpointer meta_copy_func(gpointer data, gpointer user_data)
{
    ...
    if (srcMeta->extMsgSize > 0) {
        if (srcMeta->objType == NVDS_OBJECT_TYPE_VEHICLE_EXT) {
            ...
            NvDsVehicleObjectExt *obj =
                (NvDsVehicleObjectExt *)g_malloc0(sizeof(NvDsVehicleObjectExt));
            if (srcObj->type)
            ...
            NvDsPersonObjectExt *obj =
                (NvDsPersonObjectExt *)g_malloc0(sizeof(NvDsPersonObjectExt));
...
{
    g_printerr(
        "For inference using nvinfer, Usage: %s -i <H264 filename> -p <Proto adaptor library> "
        "--conn-str=<Connection string>\n",
        bin);
    g_printerr(
        "For inference using nvinferserver, Usage: %s -t inferserver -i <H264 filename> -p <Proto "
        "adaptor library> --conn-str=<Connection string>\n",
        bin);
}
...
int main(int argc, char *argv[])
{
    ...
    if (!pipeline || !source || !h264parser || !decoder || !nvstreammux || !pgie || !nvvidconv ||
        !nvosd || !msgconv || !msgbroker || !tee || !queue1 || !queue2 || !sink) {

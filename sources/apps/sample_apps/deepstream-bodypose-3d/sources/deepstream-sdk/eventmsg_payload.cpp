                        ...
                        for (int i = 0; i < pose_meta->num_poses; i++) {
                            ...
                            if (type == 0) {
                                ...
                                for (int joint_index = 0; joint_index < pose_meta->poses[i].num_joints; joint_index++) {
                                    std::string s = _joint_maps[type][joint_index] + "," + std::to_string(pose_meta->poses[i].joints[joint_index].x) + "," + to_string(pose_meta->poses[i].joints[joint_index].y) + "," + to_string(pose_meta->poses[i].joints[joint_index].confidence);
                                    ss << s << "|";
                                }
                            } else if (type == 1) {  // BodyPose3DNet
                                ss << "|#|pose3D|";
                                for (int joint_index = 0; joint_index < pose_meta->poses[i].num_joints; joint_index++) {
                                    std::string s = _joint_maps[type][joint_index] + "," + std::to_string(pose_meta->poses[i].joints[joint_index].x) + "," + to_string(pose_meta->poses[i].joints[joint_index].y) + "," + to_string(pose_meta->poses[i].joints[joint_index].z) + "," + to_string(pose_meta->poses[i].joints[joint_index].confidence);
                                    ss << s << "|";
                                }

/*
 * SPDX-FileCopyrightText: Copyright (c) 2019-2021 NVIDIA CORPORATION & AFFILIATES. All rights
...
static gboolean extract_ms_from_utc(gchar *utc, guint32 *ms)
{
    ...
    utc_string_length = (guint32)(((size_t)utc_delim) - ((size_t)utc)) - LENGTH__AMPM_UTC - 1;
    ...
    strncpy(&utc[i_ms], (utc_delim - LENGTH__AMPM_UTC), MAX_UTC_STRING_LEN - utc_string_length);
...
struct timespec extract_utc_from_uri(gchar *uri)
{
    ...
    if (!first_char_not_processed ||
        strncmp(first_char_not_processed, URI_UTC_END_DELIM, LENGTH_UTC_END_DELIM) != 0) {

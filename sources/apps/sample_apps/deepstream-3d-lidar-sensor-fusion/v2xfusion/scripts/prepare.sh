...
function prepareEngine() {
    ...
    /usr/src/tensorrt/bin/trtexec --onnx=../models/v2xfusion/${MODEL_NAME}.onnx --plugins=${plugins} \
        --fp16 --int8 --sparsity=enable --dumpLayerInfo --exportLayerInfo=../models/v2xfusion/${MODEL_NAME}.layer.json \
        --saveEngine=../models/v2xfusion/1/${MODEL_NAME}.engine --buildOnly \
    ...
    echoGreen "Move the generated model engine file and configs to triton_model_repo"
...
function prepareDataSet() {
    if [ ! -f V2X-Seq-SPD-Example.zip ];then
        ...
        echoGreen "Download V2X-Seq-SPD-Example.zip"

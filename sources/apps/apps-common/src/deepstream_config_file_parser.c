                      ...
                      gchar *cfg_file_path)
{
    ...
    for (key = keys; *key; key++) {
        if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_TYPE)) {
            ...
            config->sensorIdToPadIdMapping = g_key_file_get_boolean(
                key_file, group, CONFIG_GROUP_SOURCE_SENSORID_PADID_MAPPING, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_VIDEO_FORMAT)) {
            config->video_format =
                g_key_file_get_string(key_file, group, CONFIG_GROUP_SOURCE_VIDEO_FORMAT, &error);
            CHECK_ERROR(error);
            ...
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_RTSP_INIT_RECONNECT_INTERVAL_SEC)) {
            config->init_rtsp_reconnect_interval_sec = g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_SOURCE_RTSP_INIT_RECONNECT_INTERVAL_SEC, &error);
            CHECK_ERROR(error);
            ...
            config->low_latency_mode = g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_SOURCE_LOW_LATENCY_DECODE, &error);
            CHECK_ERROR(error);
            ...
            config->select_rtp_protocol = g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_SOURCE_SET_RTP_PROTOCOL, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_LEAKY)) {
            config->leaky =
                g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_LEAKY, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_MAX_SIZE_BUFFERS)) {
            config->max_size_buffers = g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_SOURCE_MAX_SIZE_BUFFERS, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_BUFFER_MODE)) {
            config->buffer_mode =
                g_key_file_get_integer(key_file, group, CONFIG_GROUP_SOURCE_BUFFER_MODE, &error);
            CHECK_ERROR(error);
            ...
            config->extract_sei_type5_data = g_key_file_get_integer(
                key_file, CONFIG_GROUP_SOURCE, CONFIG_GROUP_SOURCE_EXTRACT_SEI_TYPE5_DATA, &error);
            CHECK_ERROR(error);
        }
#if defined(__aarch64__) && !defined(AARCH64_IS_SBSA)
        else if (!g_strcmp0(*key, CONFIG_GROUP_SOURCE_NVVIDEOCONVERT_COPY_HW)) {
            config->nvvideoconvert_copy_hw = g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_SOURCE_NVVIDEOCONVERT_COPY_HW, &error);
            CHECK_ERROR(error);
        }
#endif
        else {
...
gboolean parse_streammux(NvDsStreammuxConfig *config, GKeyFile *key_file, gchar *cfg_file_path)
{
    ...
    for (key = keys; *key; key++) {
        if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_COMPUTE_HW)) {
            config->compute_hw = g_key_file_get_integer(key_file, CONFIG_GROUP_STREAMMUX,
                                                        CONFIG_GROUP_STREAMMUX_COMPUTE_HW, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_SORT_BATCH)) {
            config->sort_batch = g_key_file_get_boolean(key_file, CONFIG_GROUP_STREAMMUX,
                                                        CONFIG_GROUP_STREAMMUX_SORT_BATCH, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_CACHE_BUFFER)) {
            config->buffer_cache = g_key_file_get_boolean(
                key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_CACHE_BUFFER, &error);
            CHECK_ERROR(error);
            ...
            config->sync_inputs_ntp = g_key_file_get_integer(
                key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_SYNC_INPUTS_NTP, &error);
            CHECK_ERROR(error);
            ...
            config->frame_duration = g_key_file_get_uint64(
                key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_FRAME_DURATION, &error);
            CHECK_ERROR(error);
            ...
            config->async_process = g_key_file_get_boolean(
                key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_ASYNC_PROCESS, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_STREAMMUX_DROP_PIPELINE_EOS)) {
            config->no_pipeline_eos = g_key_file_get_boolean(
                key_file, CONFIG_GROUP_STREAMMUX, CONFIG_GROUP_STREAMMUX_DROP_PIPELINE_EOS, &error);
            CHECK_ERROR(error);
...
gboolean parse_dsexample(NvDsDsExampleConfig *config, GKeyFile *key_file)
{
    ...
    for (key = keys; *key; key++) {
        if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
            ...
            config->batch_size = g_key_file_get_integer(key_file, CONFIG_GROUP_DSEXAMPLE,
...
gboolean parse_segvisual(NvDsSegVisualConfig *config, GKeyFile *key_file)
{
    ...
    for (key = keys; *key; key++) {
        if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
            ...
            config->max_batch_size = g_key_file_get_integer(
                key_file, CONFIG_GROUP_SEGVISUAL, CONFIG_GROUP_SEGVISUAL_BATCH_SIZE, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_NVBUF_MEMORY_TYPE)) {
            config->nvbuf_memory_type = g_key_file_get_integer(key_file, CONFIG_GROUP_SEGVISUAL,
                                                               CONFIG_NVBUF_MEMORY_TYPE, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GPU_ID)) {
            config->gpu_id =
                g_key_file_get_integer(key_file, CONFIG_GROUP_SEGVISUAL, CONFIG_GPU_ID, &error);
            CHECK_ERROR(error);
        } else {
                        ...
                        gchar *dewarper)
{
    ...
    for (key = keys; *key; key++) {
        if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
            config->enable =
                g_key_file_get_integer(key_file, dewarper, CONFIG_GROUP_ENABLE, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GPU_ID)) {
            ...
            config->config_file = get_absolute_file_path(
                cfg_file_path, g_key_file_get_string(key_file, dewarper,
                                                     CONFIG_GROUP_DEWARPER_CONFIG_FILE, &error));
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_NVBUF_MEMORY_TYPE)) {
            config->nvbuf_memory_type =
                g_key_file_get_integer(key_file, dewarper, CONFIG_NVBUF_MEMORY_TYPE, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_DEWARPER_NUM_SURFACES_PER_FRAME)) {
            config->num_surfaces_per_frame = g_key_file_get_integer(
                key_file, dewarper, CONFIG_GROUP_DEWARPER_NUM_SURFACES_PER_FRAME, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_DEWARPER_SOURCE_ID)) {
            guint source_id =
                g_key_file_get_integer(key_file, dewarper, CONFIG_GROUP_DEWARPER_SOURCE_ID, &error);
            config->source_id = source_id;
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_DEWARPER_NUM_OUTPUT_BUFFERS)) {
            config->num_out_buffers = g_key_file_get_integer(
                key_file, dewarper, CONFIG_GROUP_DEWARPER_NUM_OUTPUT_BUFFERS, &error);
            CHECK_ERROR(error);
        } else {
...
gboolean parse_tracker(NvDsTrackerConfig *config, GKeyFile *key_file, gchar *cfg_file_path)
{
    ...
    for (key = keys; *key; key++) {
        if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
            ...
            gchar *temp = g_key_file_get_string(key_file, CONFIG_GROUP_TRACKER,
            ...
            if (g_strv_length(configFileList) == 1) {
                ...
                for (int i = 1; i < (int)g_strv_length(configFileList); i++) {
                    single_config_file_path =
                        get_absolute_file_path(cfg_file_path, configFileList[i]);
            ...
            config->input_tensor_meta = g_key_file_get_boolean(
                key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_INPUT_TENSOR_META, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_TENSOR_META_GIE_ID)) {
            config->input_tensor_gie_id = g_key_file_get_integer(
                key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_TENSOR_META_GIE_ID, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_COMPUTE_HW)) {
            config->compute_hw = g_key_file_get_integer(key_file, CONFIG_GROUP_TRACKER,
            ...
            config->user_meta_pool_size = g_key_file_get_integer(
                key_file, CONFIG_GROUP_TRACKER, CONFIG_GROUP_TRACKER_USER_META_POOL_SIZE, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_TRACKER_SUB_BATCHES)) {
            config->sub_batches = g_key_file_get_string(key_file, CONFIG_GROUP_TRACKER,
                    ...
                    gchar *cfg_file_path)
{
    ...
    for (key = keys; *key; key++) {
        if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
            ...
            config->encoder_config.output_io_mode = (NvDsEncOutputIOMode)g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_SINK_OUTPUT_IO_MODE, &error);
            CHECK_ERROR(error);
            ...
            config->msg_conv_broker_config.broker_sleep_time = g_key_file_get_integer(
                key_file, group, CONFIG_GROUP_SINK_MSG_BROKER_SLEEP_TIME, &error);
            CHECK_ERROR(error);
        } else {
            NVGSTDS_WARN_MSG_V("Unknown key '%s' for group [%s]", *key, group);
        }
                       ...
                       gchar *cfg_file_path)
{
    ...
    for (key = keys; *key; key++) {
        if (!g_strcmp0(*key, CONFIG_GROUP_ENABLE)) {
            ...
            config->conv_dummy_payload = g_key_file_get_boolean(
                key_file, group, CONFIG_GROUP_SINK_MSG_CONV_DUMMY_PAYLOAD, &error);
            CHECK_ERROR(error);
        } else if (!g_strcmp0(*key, CONFIG_GROUP_SINK_MSG_CONV_EMBEDDING_FILTER)) {
            config->embedding_filter = g_key_file_get_string(
                key_file, group, CONFIG_GROUP_SINK_MSG_CONV_EMBEDDING_FILTER, &error);
            CHECK_ERROR(error);
        }
    }
